...
class FrontmatterGateAction:
    ...
    async def execute(self, context: EventContext) -> ActionResult:
        ...
        # only here, to interpret the zero case. Removal condition: this use goes
        # away once cf validate frontmatter --json reports which staged paths it
        # skipped as out of scope (ecorkran/context-forge#96).
        staged_count = len(context.staged_paths)
        if staged_count > 0 and files_checked == 0:
            in_scope_count = sum(
                1 for path in context.staged_paths if is_under_cf_document_root(path, context.cwd)
            )
            if in_scope_count == 0:
