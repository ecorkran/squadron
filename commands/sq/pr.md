...
nothing is appended, substituted, or removed.
...
Pass the remainder of `$ARGUMENTS` (after stripping the leading `show` word) directly to:
...
Pass the remainder of `$ARGUMENTS` (after stripping the leading `create` word) directly to:
