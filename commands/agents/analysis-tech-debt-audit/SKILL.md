<!-- Forked from github:ecorkran/tech-debt-audit (MIT). Original: github:ksimback/tech-debt-skill -->
---
name: analysis-tech-debt-audit
...
A skill that conducts a deliberate, opinionated audit of an entire codebase and produces the audit file with cited findings.

When invoked as `$analysis-tech-debt-audit`, follow the protocol below. Everything from here through the `---` divider is the protocol you execute. The section after the divider is documentation for humans installing or maintaining this skill.
...
This skill ships with squadron and is installed by squadron, not by hand:

```bash
sq install-commands --ide codex
```

That writes every squadron skill, this one included, to `~/.agents/skills/`. For a project-only
install (just this repo), add `--local` to write to `.agents/skills/` instead.

Verify it loaded by typing `$analysis-` in a Codex session and confirming the skill is offered.

## Usage

In a Codex session, in the repo you want audited:
...
**Project-level overrides.** A `.agents/skills/analysis-tech-debt-audit/SKILL.md` in a specific repo overrides the global one. Useful when a project needs custom dimensions — e.g., an agent codebase might add "prompt injection surface area" or "tool-call cost per turn" as audit categories.
...
For very large repos (>200k LOC), even subagent dispatch can produce shallow results. Consider scoping to a module: `$analysis-tech-debt-audit src/payments`.
