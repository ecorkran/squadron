---
name: sq-run
description: Runs a squadron pipeline step by step via the prompt-only executor, driving cf operations, dispatches, reviews, checkpoints and commits. Use when the user asks to run a pipeline such as slice, design or tasks.
---
...
Parse what the user typed after `$sq-run` using this three-step procedure:
