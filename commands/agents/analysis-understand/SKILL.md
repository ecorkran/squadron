...
A skill that reads an existing `understand-anything` knowledge graph and writes squadron
...
When invoked as `$analysis-understand`, follow the protocol below. Everything from here through the
`---` divider is the protocol you execute. The section after the divider is documentation for humans
installing or maintaining this skill.

Note that `/understand` throughout this document names the **upstream `understand-anything` plugin's
own command**, which produces the graph this skill reads. It is not this skill and is not run by
this skill.
...
This skill implements two flows. What the user typed after `$analysis-understand` selects between
them:

| What the user typed | Flow |
|---|---|
| nothing | Flow: Comprehension Analysis |
...
Any unrecognized text stops the same way — say it is unrecognized and stop; do not
guess at intent.
