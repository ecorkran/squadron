...
- `.agents/skills/`, `.claude/skills/` — Agent Skills (`SKILL.md` folders)
...
These files are managed by Context Forge and should not have project frontmatter fields added to them. In `CLAUDE.md`, `AGENTS.md` and `.github/copilot-instructions.md`, only the region between `<!-- BEGIN:context-forge -->` and `<!-- END:context-forge -->` is managed; content outside it is yours and survives re-runs. `.github/instructions/` files carry a `<!-- context-forge:generated -->` stamp and are regenerated wholesale.
