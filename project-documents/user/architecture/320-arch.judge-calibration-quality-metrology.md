...
- **Initiative 340 (Skill Pack Infrastructure)** — ships the analysis pack containing the tech-debt-audit skill this component's code-quality oracle runs. See [340-arch.skill-pack-infrastructure.md](340-arch.skill-pack-infrastructure.md). **Skill-name drift, resolved in favor of `tech-debt-audit`:** the shipped artifact's frontmatter `name:`, its file (`commands/analysis/tech-debt-audit.md`), and the live dispatch surface (`/analysis:tech-debt-audit`) all use `tech-debt-audit`; 340-arch's `tech-debt-analyze` is the stale name that never matched the shipped skill. This document uses the real identifier; 340-arch is the doc that should be reconciled (a 340/900 housekeeping item, not a 320 change).
