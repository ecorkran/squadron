...
| 323 — Tech-Debt-Audit Baseline Harness | **complete** | [`323-slice.tech-debt-audit-baseline-harness.md`](../slices/323-slice.tech-debt-audit-baseline-harness.md) | [`323-tasks.tech-debt-audit-baseline-harness.md`](../tasks/323-tasks.tech-debt-audit-baseline-harness.md) | Cross-project tech-debt-audit baseline plus a measured, per-project noise floor (`sq metrology audit run\|variance`, `sq metrology report baseline`). |
| 324 — Pre-Emption Prompt & Delta Measurement | **complete** | [`324-slice.pre-emption-prompt-delta-measurement.md`](../slices/324-slice.pre-emption-prompt-delta-measurement.md) | [`324-tasks.pre-emption-prompt-delta-measurement.md`](../tasks/324-tasks.pre-emption-prompt-delta-measurement.md) | Depends on 323. Generates a static pre-emption fragment for dispatch (`sq metrology preempt generate`) and a floor-relative before/after delta report (`sq metrology audit delta`). |

**All five anticipated slices are complete:** the human-oracle chain (320 → 321 → 322), the audit oracle's data half (323), and its intervention (324). Initiative 320 is finished. One verification gap is recorded rather than papered over: 324's live `audit delta` run could not execute inside a Claude Code session (the audit harness spawns the Claude Code CLI, which refuses to nest), so the delta path is covered by fixture and stubbed-harness tests and its end-to-end run is documented in 324's Verification Walkthrough as outstanding.
...
**Audit oracle** — the counterpart to the human oracle (slices 323/324): a different, automated way of judging judge quality, using tech-debt-audit signal instead of a human verdict. Shares only 320's store and trend conventions with the human-oracle chain — not a report path. Both are complete: 323 (data + variance) and 324 (intervention).
...
**Human oracle** — the ground-truth source for slices 320-322: a human reviewer's blind verdict, captured and compared against a judge's verdict. Contrast with the audit oracle (323/324, complete).
