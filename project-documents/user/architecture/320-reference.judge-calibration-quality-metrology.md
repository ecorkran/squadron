...
| 324 — Pre-Emption Prompt & Delta Measurement | **designed** | [`324-slice.pre-emption-prompt-delta-measurement.md`](../slices/324-slice.pre-emption-prompt-delta-measurement.md) | — | Depends on 323. Generates a static pre-emption fragment for dispatch and a floor-relative before/after delta report. |

**The human-oracle chain (320 → 321 → 322) is done, and 323 (the audit oracle's data half) is done.** 324 (the audit oracle's intervention) is designed but not yet implemented. This is why the parent architecture doc (`320-arch...md`) and slice plan (`320-slices...md`) both carry frontmatter `status: in_progress`, not `complete` — the initiative as a whole is not finished until 324 ships.
...
**Audit oracle** — the counterpart to the human oracle (slices 323/324): a different, automated way of judging judge quality, using tech-debt-audit signal instead of a human verdict. Shares only 320's store and trend conventions with the human-oracle chain — not a report path. 323 (data + variance) is complete; 324 (intervention) is designed, not yet implemented.
