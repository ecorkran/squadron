...
17. [ ] **(157) PreCompact Hook for Interactive Claude Code** — Ship a `PreCompact` hook script and install mechanism so that when a user types `/compact` (or auto-compaction fires) inside an interactive Claude Code session (VS Code extension or CLI Claude Code), squadron injects pipeline-aware or project-aware compact instructions before the conversation is summarized. This is a `.claude/settings.json` shell hook — completely independent of SDK mode and the Agent SDK `hooks` field. Includes: a `sq compact-instructions` CLI subcommand (or equivalent) that the hook invokes to emit instructions on stdout, instruction resolution rules (active pipeline state if any, otherwise project-level default template), an installer (`sq hooks install` or documented manual snippet) that adds the entry to `.claude/settings.json` without clobbering existing hooks, and matcher coverage for both manual `/compact` and auto-compact triggers. Dependencies: [157]. Risk: Low. Effort: 2/5
