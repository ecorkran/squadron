...
10. [x] **(149) Pipeline Executor and Loops** — Step sequencer that expands step types into action sequences and executes them in order. Basic loops: fixed iteration with max count and until condition evaluated against step output. Collection loops: `each` construct that iterates over items from a typed query source (initially CF queries for unfinished slices, extensible). Convergence loop syntax: `loop.strategy` field parsed and acknowledged, but execution falls back to basic max-iteration behavior (convergence strategies are 180 scope). **Design decisions required:** (1) collection loop item binding semantics — how `{slice.index}` resolves inside an `each` block (item type/schema from CF query, field traversal syntax, missing field behavior, read-only binding); (2) loop condition grammar — what `until: review.pass` means (field path into ActionResult? named property? formal expression syntax?), how the validator checks conditions, and how the executor evaluates them. See OQ5 and loop condition finding in `140-arch.pipeline-foundation.md`. Dependencies: [147, 148]. Risk: Medium (loop semantics, collection variable binding). Effort: 3/5
...
14. [x] **(155) SDK Pipeline Executor** — Full pipeline automation via `ClaudeSDKClient`. Dispatch steps spawn a persistent SDK session that executes work autonomously with tool use (file I/O, commands, web search). Per-step model switching via `client.set_model(model)`. Server-side context compaction via the `context_management` API (`compact_20260112` beta) with pipeline-defined `instructions` from compact templates and configurable `trigger` thresholds. Reviews execute normally (SDK can spawn subprocesses). Checkpoints pause and optionally notify. Runs outside Claude Code sessions only (`sq run slice 154` from straight CLI). This is the fully automated path — prompt-only mode (slices 153-154) remains the interactive bridge for IDE and Claude Code CLI sessions. Dependencies: [154]. Risk: Medium (SDK API stability, compaction beta). Effort: 3/5

15. [x] **(156) Pipeline Executor Hardening** — Fix resume-with-correct-mode: add `execution_mode` enum field to `RunState` (schema v2) so resume always uses the same runner as the original run — no hardcoded mode-name strings, dispatch via enum match. Fix implicit resume and `--resume` paths to both honour the stored mode. Normalize pipeline names to lowercase at load time and CLI input boundary (case-insensitive lookup and state matching). Dependencies: [155]. Risk: Low. Effort: 2/5

16. [x] **(157) PreCompact Hook for Interactive Claude Code** — Ship a `PreCompact` hook so that when a user types `/compact` (or auto-compaction fires) inside an interactive Claude Code session (VS Code extension or CLI Claude Code), squadron injects project-aware compact instructions before the conversation is summarized. This is a `.claude/settings.json` shell hook — completely independent of SDK mode and the Agent SDK `hooks` field. Parts: (a) a hidden `sq` subcommand (not shown in `--help`, purely the hook's invocation target) that reads config to pick a template or literal, loads the YAML template from `src/squadron/data/compaction/` (reusing the existing compaction-template loader), renders params (`{slice}`, `{project}` — resolved from CF project state, missing values render empty, hook never errors), and emits the hook output shape on stdout. User-facing surface is config only: `sq config set compact.template minimal` or `sq config set compact.instructions "..."`; (b) two new config keys `compact.template` (template name, default `minimal`) and `compact.instructions` (literal override string), mutually exclusive, both honouring the existing user / `--project` layering; (c) extend `sq install-commands` (and `uninstall-commands`) to also write/remove the `PreCompact` entry in `.claude/settings.json`, merging non-destructively with existing hooks; (d) matcher covers both manual `/compact` and auto-compact triggers. Dependencies: [CF project state reader]. Risk: Low. Effort: 2/5. **Design Complete: [157-slice.precompact-hook-for-interactive-claude-code.md](user/slices/157-slice.precompact-hook-for-interactive-claude-code.md)**

17. [x] **(158) SDK Session Management and Compaction** — Controlled context compaction at pipeline step boundaries via session rotate: switch to cheap summarizer model in current session, query with compact template instructions, capture summary, disconnect, start fresh session with summary as context. Replaces the unconnected `configure_compaction()` stub from slice 155. Add optional `model` field to compact step YAML for cheaper summarization. The Agent SDK (`ClaudeSDKClient`) does not expose `context_management` or `compaction_control` — session rotate is the only path to deterministic, step-boundary compaction. Dependencies: [155, 156]. Risk: Medium (session lifecycle edge cases, state continuity across reconnects, summary quality). Effort: 3/5. **Design Complete: [157-slice.sdk-session-management-and-compaction.md](user/slices/157-slice.sdk-session-management-and-compaction.md)**

18. [x] **(160) Interactive Checkpoint Resolution** — Replace pause-and-exit checkpoint behavior with an interactive prompt offering three options: (1) **Accept** the suggested resolution from the prior review/finding (if any) and continue in-process, (2) **Override** with custom instructions entered at the prompt and continue in-process, (3) **Exit** the pipeline and resume later via `sq run <pipeline> <index>` (current behavior). Options 1 and 2 avoid the full disconnect/persist/exit/resume cycle, keeping the SDK session live for fast iteration on reviewable artifacts. Applies to both prompt-only and SDK execution modes. Includes display of the triggering review verdict and any extracted suggestion text so the user has context before deciding. Dependencies: [156]. Risk: Low. Effort: 2/5. **Design Complete: [160-slice.interactive-checkpoint-resolution.md](../slices/160-slice.interactive-checkpoint-resolution.md)**

19. [x] **(161) Summary Step with Emit Destinations** — New `summary` pipeline step type that captures a summary of the current SDK session's conversation-so-far and emits it to one or more destinations. Factors the summary-generation axis out of the existing `compact` step: `summary` generates text via the same session-rotate summarizer mechanism (cheap-model switch, compact-template dispatch, summary capture) but decouples *what to do with the summary* from *how to make it*. Introduces an `emit` field taking a list of destinations from a registry: `stdout`, `file: <path>`, `clipboard` (via `pyperclip`), and `rotate` (end current SDK session, start fresh, seed with the summary — the current `compact` behavior). Default `emit: [stdout]`. A `checkpoint: always` shorthand on the step pauses after emit so the user can paste into Claude Code to discuss paused artifacts. `compact` is retained as a distinct alias that hardcodes `emit: [rotate]` for pipeline authors who don't want to think about destinations. Solves the "I want to discuss my paused pipeline's design in interactive Claude Code without cut-and-paste gymnastics" gap — paired with `checkpoint: always`, the summary reaches the clipboard and the user pastes it into a fresh conversation. Thinking blocks are filtered like current dispatch. `emit: [rotate]` outside SDK mode is a validation error (warning-level if easy, since session rotate is SDK-only by design). Known follow-up: a `clear` equivalent (rotate without seeding) can be layered on as either a second alias or a `seed: false` option if demand emerges. Dependencies: [158]. Risk: Low (extends established compact mechanics, new code is mostly an emit registry + step wiring). Effort: 2/5. **Design Complete: [161-slice.summary-step-with-emit-destinations.md](../slices/161-slice.summary-step-with-emit-destinations.md)**

20. [x] **(162) /sq:summary — Clipboard Summary for Manual Context Reset** — New `/sq:summary [template]` slash command and supporting `sq summary-instructions` CLI that lets the user produce a custom, template-driven summary of the *current interactive Claude Code conversation* and copy it to the clipboard, so they can `/clear` and paste it into a fresh context. Addresses the unreliability of `/compact [with instructions]`, which is non-deterministic — sometimes honors custom instructions, sometimes ignores them. Reuses the existing compaction template machinery: `load_compaction_template`, `render_with_params`, CF best-effort param gathering (`slice`, `phase`, `project`), and the `compact.template` config key as the default. The slash command expands to a prompt that (1) shells out to `sq summary-instructions` to fetch rendered template instructions, (2) instructs the assistant to produce the summary inline following those instructions, and (3) pipes the result to clipboard via `pbcopy` (macOS) with `xclip`/`wl-copy` fallback (Linux). Clipboard-only output with a one-line confirmation — no chat dump. No new SDK session plumbing; the current Claude Code session is the summary target, and squadron only supplies instructions and a clipboard sink. Windows clipboard deferred. Dependencies: [158 for template/render machinery reuse]. Risk: Low. Effort: 1/5. **Design Complete: [162-slice.sq-summary-clipboard-summary-for-manual-context-reset.md](../slices/162-slice.sq-summary-clipboard-summary-for-manual-context-reset.md)**

21. [x] **(163) Pipeline Run Summary Persistence and Restore** — Default file destination for summary step `emit: [file]` when no explicit path is given: writes to `~/.config/squadron/runs/summaries/{project}-{pipeline}.md` (project name resolved from CF, pipeline name from run state). Overwrites on each run (latest-only). Extends `/sq:summary --restore` to read the most recent summary for the current project and emit it as a context-seeding block — no run-id required. Supporting hidden CLI subcommand resolves project name from CF via CWD, lists matching summary files, and returns content. Prompt-only `run.md` summary handler updated to write to the same conventional path via Bash. Closes the "run a pipeline in CLI terminal, restore context in VS Code" workflow gap. Dependencies: [161, 162]. Risk: Low. Effort: 1/5
...
Feature:
  153. Prompt-Only Pipeline Executor                     (after 151) ✓ complete
  155. SDK Pipeline Executor                              (after 153) ✓ complete
  156. Pipeline Executor Hardening                        (after 155) ✓ complete
  157. PreCompact Hook for Interactive Claude Code        ✓ complete
  158. SDK Session Management and Compaction              ✓ complete
  160. Interactive Checkpoint Resolution                  (after 156) ✓ complete
  161. Summary Step with Emit Destinations                (after 158) ✓ complete
  162. /sq:summary Clipboard Summary                      (after 158) ✓ complete
  163. Pipeline Run Summary Persistence and Restore      (after 161, 162) ✓ complete
...
6. [FUTURE] **(154) Prompt-Only Loops** — Extend prompt-only mode with `each`/collection loop support (executor returns successive iteration instructions via `--next`, caller doesn't need to know it's a loop). Would enable design-batch pipelines in prompt-only mode. Deprioritized: the `/sq:summary --restore` workflow (slices 162-163) now covers the CLI↔IDE context handoff that prompt-only loops were meant to bridge, and prompt-only mode is primarily a pipeline-testing surface. Design is complete if we revisit. Dependencies: [153]. Effort: 2/5. **Design preserved: [154-slice.prompt-only-loops.md](../slices/154-slice.prompt-only-loops.md)**

7. [FUTURE] **Implicit `--param` Expansion** — Unknown `--foo bar` flags on `sq run` that don't match a known option should be treated as `--param foo=bar` implicitly, rather than raising an error. Reduces friction for pipelines with multiple required params: `sq run test-review 154 --template slice` instead of `sq run test-review 154 --param template=slice`. Known options take precedence; ambiguity (e.g. `--validate`) resolves to the known option. Dependencies: [151]. Effort: 1/5

8. [FUTURE] **(165) Agent-Context Interactive Checkpoints** — Interactive checkpoint resolution (Accept / Override / Exit) is currently CLI-only. When a pipeline runs via `/sq:run` or any other agent-turn executor, `_is_interactive()` returns false (no TTY) and the checkpoint falls through to immediate PAUSED. Supporting interactive checkpoints in an agent context requires a tool-call-based checkpoint protocol: the executor suspends, surfaces a structured decision request back to the orchestrating agent turn, the agent presents options to the user (findings, verdict, three choices), and on reply injects `override_instructions` into the resumed run state via `--resume`. Distinct from stdin-based resolution — requires coordination between executor, run-state schema, and the `/sq:run` skill layer. Dependencies: [160, 151]. Effort: 3/5

9. [FUTURE] **`/sq:summary --restore` Improvements** — Two gaps in the restore workflow: (a) **Summary selection by key** — when multiple summaries exist for a project (e.g. `p4`, `p5`), `--restore` always picks most-recent; `/sq:summary --restore p4` should select by key. Requires a `--key` flag on `sq _summary-instructions` and argument threading in the skill prompt. (b) **Clipboard summaries not restorable** — summaries generated interactively (clipboard path) are not written to disk, so they are unavailable to `--restore` in a new session. Interactive runs should also write to the conventional path (`~/.config/squadron/runs/summaries/{project}-{template}.md`) alongside pipeline-run summaries. Both items are additive; no breaking changes to existing restore behavior. Tracked: [ecorkran/squadron#7](https://github.com/ecorkran/squadron/issues/7). Dependencies: [162, 163]. Effort: 1/5

10. [FUTURE → 180] **(159) Pipeline Fan-Out / Fan-In Step Type** — General-purpose parallel branch infrastructure: a new `fan_out` step type that dispatches N copies of an inner step config concurrently (each with its own resolved model from a model pool or explicit list), waits for all to complete via `asyncio.gather`, then passes the collected results through a `fan_in` reducer. **Relocated to initiative 180 (Pipeline Intelligence).** Fan-out is the infrastructure that 180's ensemble review (`unanimous` convergence strategy, multi-model agreement with boost weights) depends on, and it shares the model-pool machinery that 180 owns. Splitting fan-out from pools and ensemble review across two initiatives would fragment one coherent feature. Will be slotted into 180's slice plan when that plan is drafted. Dependencies: [157, 180 model pools]. Effort: 3/5
...
- **Convergence loop strategies are 180 scope.** Slice 149 parses the `loop.strategy` YAML field and applies basic max-iteration fallback. Strategy registration and weighted-decay execution are 180 (Pipeline Intelligence) deliverables that plug into 149's extension point.
- **Model pools are 180 scope.** The model resolver in slice 142 recognizes the `pool:` prefix but raises a clear error ("model pools require Pipeline Intelligence initiative"). Pool resolution logic is a 180 deliverable.
- **Slice 154 (Prompt-Only Loops) deferred to Future Work.** The design exists but the use case was largely displaced by slices 162-163 (`/sq:summary` + `--restore`). Prompt-only mode remains useful as a pipeline-testing surface, but each-loop support is no longer on the critical path.
- **Slice 159 (Fan-Out / Fan-In) relocated to initiative 180.** Fan-out is the infrastructure ensemble review and multi-model consensus depend on, and it shares model-pool plumbing with 180. It will be slotted into 180's slice plan when that plan is drafted rather than living in 140 as an orphan. Slice 160 (Interactive Checkpoint Resolution) remains in 140 — it is deterministic UX over existing checkpoint mechanics, not intelligence.
