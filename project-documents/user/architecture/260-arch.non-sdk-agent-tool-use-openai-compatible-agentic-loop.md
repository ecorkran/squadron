...
**Scope.** OpenAI-compatible providers only (openrouter, openai, local, gemini). Action coverage is `dispatch` plus review — both the pipeline `review`/`summary` actions and the standalone review client path (`run_review_with_profile`), which already threads `template.allowed_tools` into `AgentConfig` but is silently ignored by non-SDK providers today (issue #68). Reviews use a read-only tool subset. SDK provider is unchanged — it already has full tool support via the `claude_code` system-prompt preset and `bypassPermissions`. Codex provider is out of scope (it has its own internal agentic handling).

**Motivation.** Pipelines should be able to use cost-effective or capability-specific non-SDK models for tasks that require file I/O (design, codegen, refactoring) without being forced to use Claude. The same gap constrains reviews: a non-SDK reviewer today receives only what `_inject_file_contents` stuffs into the prompt up front and cannot request surrounding context mid-review. This unlocks the original promise of profile-aware dispatch (slice 240): pick the right model for the step, and the step actually works.
...
- `dispatch` action wiring reads `allowed_tools` from the resolved step config and passes it to `AgentConfig`. Review coverage follows as its own slice: when the provider supports tool use and the template allows a read-only tool set, the review path skips full-file injection (`_inject_file_contents`) while still injecting the diff — the diff anchors the review; the model pulls referenced files on demand. With no tools allowed, injection behavior is unchanged.
...
- **Cache-friendly history: append-only, stable prefix.** Each loop iteration re-sends the full accumulated history, and each request is a strict prefix-extension of the previous one — exactly the shape automatic prefix caching matches (OpenAI, DeepSeek, and Moonshot cache automatically; OpenRouter passes provider caching through). To keep that discount, the loop must never mutate earlier history entries between iterations — no injected timestamps, no re-rendered file contents, no rewritten system prompt. Anything mutable goes in new messages only.
- **Review-path injection interplay.** `run_review_with_profile` currently injects full file contents when `capabilities.can_read_files` is false. Tool-enabled reviews need a capability signal (e.g. a `supports_tool_use` field on `ProviderCapabilities`, or `can_read_files` made config-dependent) so the review client can skip file injection while keeping diff injection. Decided in the review-coverage slice, not here — but the capability flag should not encode provider identity.
...
- **(265) Review coverage — standalone client and pipeline actions.** In scope (promoted 20260824; previously a deferred extension). Activate `allowed_tools` on the review path: the standalone review client (`run_review_with_profile`) already passes `template.allowed_tools` into `AgentConfig` — silently ignored by non-SDK providers (issue #68) until slice 262 lands; this slice adds the injection-skip logic (tools enabled → inject diff only, model reads files on demand) and applies the same plumbing to the pipeline `review`/`summary` actions. Reviews use a read-only tool subset (`read_file`; no `write_file`, no `bash`).
...
- `src/squadron/review/review_client.py`: standalone review path — already passes `template.allowed_tools` into `AgentConfig`; `_inject_file_contents` is the compensation this initiative partially retires (issue #68).
