...
7. [x] **(306) Review Resolution — Recording That Findings Were Addressed** — [Issue #51](https://github.com/ecorkran/squadron/issues/51). A FAIL review's findings get addressed, the fix lands, and the artifact still reads `verdict: FAIL` — so downstream gating stays closed on a verdict no longer true of the code, with no mechanism to say so. Belongs here because 305 already built the decision procedure; this slice is about making it reachable and recordable outside a pipeline loop.
   ...
   **Explicitly out of scope:** restoring an agent's ability to edit `verdict:`. The original verdict stays as written, whatever else is added.
   ...
   - **Implemented:** 20260803, branch `306-slice.review-resolution-recording-that-findings-were-addressed` — fifteen commits across two task files. `sq review resolve <index> [TYPE]` ships (`/sq:review resolve` for parity), writing `{index}-resolution.{type}.{name}-r{n}.md` with `docType: review-resolution` and a top-level `resolution:` field; the review file is read and never written. `reviewedSha:` is stamped by both persistence paths at authoring time, with a file-history fallback (WARNING) for reviews written before it. Part D's archive-on-overwrite guard covers `save_review_file` *and* `save_review_result` — the CLI path uses the latter, so guarding only the former would have left the slice's own success criterion unmet.

     Three things came out differently from the design. The `review/addressed/` relocation went further: `RoundDiff`, the git measurement, and the two context-free screens moved there too, because the resolve path must *construct* a `RoundDiff` and the `TYPE_CHECKING` shim that held for annotations cannot. The reviews directory is excluded from the diff measurement — a review is written after the commit its own `reviewedSha` names, so counting it would make the empty-diff screen unreachable. And 305 does not apply the `review.max_total_injection_bytes` cap to the judge's change-set input; per T25 the check was added on this path only and the gap in 305 is tracked as issue #53 (low priority) rather than fixed here.

     Still open: T39's Context Forge coordination note has no agreed delivery mechanism (issue, message, or shared doc) — a PM decision, not implementation.
