...
Fixes [issue #42](https://github.com/ecorkran/squadron/issues/42), [issue #43](https://github.com/ecorkran/squadron/issues/43), and [issue #45](https://github.com/ecorkran/squadron/issues/45). Three defects on the multi-step `loop:` path, bundled because Parts A and B share a single 60-line function (`_execute_loop_body`) and one test file. Together #42 and #43 make "the loop fixes the findings and re-reviews" false for the demo pipeline. Each part is small because the machinery already exists — this is wiring, not new architecture. [Issue #44](https://github.com/ecorkran/squadron/issues/44) was scoped here initially and split out to its own slice; see Future Slices below.
...
1. [ ] **(911) Loop Iteration Versioning and Review Evidence** — Split out of 910 during scoping: it is a document-contract question, not a loop-execution one, and the last item below is a correctness question about whether reviews mean anything across iterations. Covers [issue #44](https://github.com/ecorkran/squadron/issues/44) plus three problems not recorded there. Four parts, roughly increasing in difficulty:

    1. **Commits between iterations (#44).** Each iteration overwrites the previous artifact with no commit, so a converging loop leaves nothing to diff and no way to recover a better earlier attempt. Diagnostically coupled to 910's Part A: without per-round history there is no evidence the findings actually landed, and a byte-identical round — an empty commit — is exactly the Part A symptom. A `commit` step type already exists. Open: automatic vs. explicit body step, every-round vs. on-change, message format, opt-in flag so existing loops do not begin writing history unexpectedly. Mechanical.

    2. **No version indicator on the artifact.** Not in #44. Open a slice design document and nothing says whether it is round 1 or round 3, or which verdict it last survived. Proposed: a simple integer `version: {n}` in frontmatter — not semver. Squadron writes frontmatter routinely, so the mechanism exists. Mechanical.

    3. **Overwrite vs. modify, and what survives a round.** A contract decision, not an implementation one: is each iteration a clean regeneration with the version tag as the only carryover, or does content persist across rounds? Simplest-that-works favors clean regeneration, and simplest is usually right for a contract other tools read. Note the seam: artifact frontmatter is the documented boundary with Context Forge review-gating, so a new field is a cross-tool contract change, not a squadron-local one.

    4. **Do review notes carry forward, and may a reviewer see the prior version?** The hard one, and it is an evidence-integrity question rather than a document one. Showing the reviewer prior findings lets it verify "was this actually addressed?" — exactly what round 2 should ask. But it also anchors: a reviewer handed a list tends to grade against that list, leaving real-but-unlisted problems unfound and accepting "addressed" claims at face value. That is the failure shape of [issue #32](https://github.com/ecorkran/squadron/issues/32) — a confident verdict over work nothing actually examined. Possible resolution: keep them separate — a clean-eyes review of the artifact, plus a distinct "were the prior findings addressed?" check that is allowed to see history — so neither contaminates the other. Needs a design conversation before implementation.

    Dependencies: [910 (Part A defines what an iteration produces)]. Risk: Medium. Effort: 3/5.
