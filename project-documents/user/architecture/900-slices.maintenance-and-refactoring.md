...
Deliberately **not** included: [issue #25](https://github.com/ecorkran/squadron/issues/25) (XML-delimit substituted documents) is prompt hardening with no reported symptom and wants its own verification against live runs; [issue #27](https://github.com/ecorkran/squadron/issues/27) (fabricated review content, unreproducible, seen 2x) has no root cause and no reproduction — Part C's telemetry and the issue's own suggested write-path tracing are the instrumentation that would catch a recurrence, so re-assess #27 after this slice rather than scoping it in blind.

Sequence E → A → B → C → D. E and A are contained. B and C both concern how a degraded parse presents itself and are best held in view together. D carries the `ReviewFinding` decision.

**Status:** not started · **Risk:** Medium (Part A adds a commit-path gate; Part D may add a `ReviewFinding` field) · **Effort:** 4/5 · **Dependencies:** [916 — sequencing only, to avoid contending for the same review-path files]
---
