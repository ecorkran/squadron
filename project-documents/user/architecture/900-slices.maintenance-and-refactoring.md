...
**Part B — empty filtered scope reports PASS (#62, High).** When `diff_exclude_patterns` ([code.yaml](src/squadron/data/templates/code.yaml)) filters every changed file out of the range, the reviewer correctly reports that nothing was in scope — at severity `note`, which leaves `verdict: PASS`. Observed clearing a review gate on slice 362 with zero lines examined. Not markdown-specific: any all-`.toml`/`.json`/`.csv` range does the same. Requires a design decision the slice must state and justify: a distinct verdict/outcome for "nothing was in scope" versus refusing to persist and exiting non-zero. A new `Verdict` member touches every gate that switches on it (`LoopCondition.REVIEW_CONCERNS_OR_BETTER`, `CheckpointTrigger.ON_CONCERNS`, verdict rollup at [review.py:279-284](src/squadron/cli/commands/review.py#L279-L284)) and is the wider blast radius of the two.
