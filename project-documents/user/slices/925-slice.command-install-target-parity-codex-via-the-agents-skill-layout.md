...
4. Bundle restructure: `commands/claude/{sq,analysis}/*.md` (today's files, moved) and `commands/agents/<skill>/SKILL.md` (new, checked in). `pyproject.toml` force-include and `_get_commands_source()` follow.
...
7. `sq doctor`: `check_slash_commands` reports per detectable target. `sq setup`: gains `--ide`, forwards it.
...
src/squadron/cli/commands/doctor_checks.py  MOD  check_slash_commands -> list[CheckResult], one per detectable target
src/squadron/cli/commands/setup.py      MOD  --ide, forwarded
src/squadron/cli/commands/setup_install.py  MOD  _install_sq_commands(target)
commands/claude/sq/*.md                 MOVED from commands/sq/
commands/claude/analysis/*.md           MOVED from commands/analysis/
commands/agents/sq-<name>/SKILL.md      NEW  one per commands/claude/sq/<name>.md
commands/agents/analysis-<name>/SKILL.md (+ agents/openai.yaml)  NEW  one per commands/claude/analysis/<name>.md
```

`targets.py` owns the vocabulary and the table. `install.py` owns the copy/receipt/stale-removal loop it has today, parameterized by a `TargetDelivery` instead of a hardcoded path and glob. Nothing in `install.py` mentions `.claude` or `.agents` after this slice.
...
3. Source = `_get_commands_source() / delivery.bundle_subdir` (`commands/agents/`).
4. Layout writer walks the source: for the agents layout, each `<skill>/` directory is copied whole (`SKILL.md` and, when present, `agents/openai.yaml`); for the Claude layout, `<sub>/*.md` as today. Both return `files_written` relative to the destination.
...
**D6 — Uninstall removes from `receipt.destination`.** Today's uninstall computes `target_dir / relative` from the flag, ignoring the destination the receipt recorded. With `--local` the two can differ (uninstall run from a different directory). The receipt's destination is the truth about where files went; `--target` on uninstall becomes a check ("receipt says X, you said Y") rather than a source of paths. For the default path this changes nothing.
...
- Doctor check names: `slash commands` (unchanged, Claude) and `codex skills` (agents), both `required=False`, `section=SECTION_INSTALL`; fix hints `sq install-commands` and `sq install-commands --ide codex`.
- The drift test reads both trees from the bundle source and asserts a bijection: `commands/claude/<sub>/<name>.md` ↔ `commands/agents/<sub>-<name>/SKILL.md`, and that each agents `name:` equals its directory.

## Implementation Details

### Migration Plan

- **Moved:** `commands/sq/` → `commands/claude/sq/`, `commands/analysis/` → `commands/claude/analysis/` (`git mv`, no content change). `pyproject.toml` force-include `"commands" = "squadron/commands"` is unchanged (it maps the whole directory). `_get_commands_source()` unchanged; callers append `delivery.bundle_subdir`.
- **Consumers:** `tests/cli/test_install_commands.py` (`test_all_command_files_exist_in_source`, `test_command_files_reference_correct_subcommand`, and the fixtures that build a fake `commands/` tree) update paths. DEVLOG references to `commands/sq/` are historical and stay.
- **Behavior preserved:** `sq install-commands` with no flags writes the same files to `~/.claude/commands/{sq,analysis}/` under the same receipt name. A test runs the pre-slice and post-slice install into `tmp_path` and asserts identical file sets and receipt contents.
...
`--ide` default `claude`. `--local` and `--target` together: `--target` wins and `--local` is reported as ignored (not silently). Unknown `--ide` exits 2 with `typer.BadParameter` naming the accepted values.
...
8. **Drift guard.**
   Add `commands/claude/sq/foo.md` with no twin; `pytest tests/cli/test_install_commands.py -k drift` fails naming `sq-foo/SKILL.md` as missing.
...
2. Bundle move (`git mv`) and test-path updates; run the existing install tests — they must pass unchanged in behavior before any new flag exists.
3. Thread `TargetDelivery` through `install.py`; add `--ide`/`--local`; per-target receipts; D6. Tests for each.
4. Author `commands/agents/` — twelve skills, prose argument handling per D3, `openai.yaml` for the two `analysis` skills. Drift test.
5. Doctor and setup. Stub `shutil.which` in tests.
6. Docs, CHANGELOG, live walkthrough under Codex, file the two follow-up issues.
