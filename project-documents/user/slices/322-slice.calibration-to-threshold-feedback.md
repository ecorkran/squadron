...
| **threshold read** | malformed `judge:` block (non-numeric floor) | fall back to `resolve_thresholds`' documented merge; flag the target | WARNING naming the template | malformed judge block → flagged, not silently defaulted |
