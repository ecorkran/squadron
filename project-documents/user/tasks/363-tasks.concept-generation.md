...
- [x] **0.1 Create the slice branch** — Effort: 1/5
  - [x] Confirm working tree is clean and current branch is `main` (integration branch is unset;
        target is `main`).
  - [x] `git checkout -b 363-slice.concept-generation main`
  - [x] Success: on the new branch, clean tree.

- [x] **0.2 Re-verify the design's premises** — Effort: 1/5
  - [x] Run the checks from Verification Walkthrough step 1 (design lines 384–392):
  ...
  - [x] Additionally confirm `pyproject.toml` still carries `[tool.ruff]` and
        `[tool.pytest.ini_options]` (the lint/test signals the design measured).
  - [x] **STOP condition:** if any check diverges from the Verified facts table (design lines
...
- [x] **1.1 Add the flow selector to the skill** — Effort: 2/5
  - [x] In `commands/analysis/understand.md`, extend the argument handling: no argument or
        ...
        until 364.
  - [x] State explicitly: selection is by explicit argument only — the skill never infers a flow
        from repo state; the absence of a concept document never auto-triggers this flow.
  - [x] State that preflight runs in full for both flows, unchanged — including the shared
        contract's `.gitignore` hygiene write. The concept flow adds no hygiene behavior and
        skips none.
  - [x] Add the empty sibling section shell "Flow: Concept Generation" after Flow: Comprehension
        Analysis, before the human-documentation divider (Implementation Notes, design line 443).
  - [x] Success: selector covers all four argument cases; explicit-only rule and preflight
        statement present; section shell in the stated position.

- [x] **1.2 Verify selector text** — Effort: 1/5
  - [x] Check the selector against design lines 137–148: routing table matches case-for-case;
        no additional recognized argument introduced.
  - [x] Success: exact case coverage confirmed. (Live selection behavior is exercised in
        Task 8.5.)
  - [x] Commit: `feat: add concept flow selector to understand skill`

## Task 2: Preconditions and the /cf:onboard boundary

- [x] **2.1 Author the preconditions** — Effort: 2/5
  - [x] Precondition 1 — graph present: the 361 preflight, executed unchanged. Reference the
        shared contract; do not duplicate it.
  - [x] Precondition 2 — ai-project-guide installed: the concept guide is read at write time from
  ...
  - [x] Precondition 3 — resolvable project name from the cf project registration, never from the
  ...
  - [x] State the boundary: `/cf:onboard` owns project setup and the greenfield conversational
        concept path; this flow owns the artifact-derived brownfield path. They compose and do
        not overlap.
  - [x] Success: three preconditions with their two terminal-stop behaviors; boundary statement
        present; no fallback language anywhere.

- [x] **2.2 Verify preconditions against this repo** — Effort: 1/5
  - [x] Confirm the guide tree path is readable, the cf registration resolves `squadron`
        (`cf get` Name field), and the graph's `project.name` is `squadron-ai` — the divergence
        the output conventions must state.
  - [x] Success: all three confirmed on this repo.
  - [x] Commit: `feat: add concept flow preconditions and onboard boundary`

## Task 3: The three-source extraction model

- [x] **3.1 Author the graph source** — Effort: 2/5
  - [x] Sources: `project.description`, `.languages`, `.frameworks`; `layers[]` names +
  ...
  - [x] Read discipline: 362's, unchanged — field-scoped `jq` selections only, whole graph never
        loaded, no `function`/`class` node read. State that this flow reads strictly less of the
        graph than the comprehension flow.
  - [x] Success: field list, inline attribution rule, and discipline statement present.

- [x] **3.2 Author the repo-prose source** — Effort: 2/5
  - [x] README resolution: the root-level README, case-insensitive, `README.md` preferred when
        several exist. Nothing below the root is read — docs/ mining is excluded.
  - [x] Attribution: claims cite their file (`README.md`); quoted material is quoted, never
        paraphrased into squadron's own voice.
  - [x] No-README fallback: affected sections fall back to graph fields where mapped, and to gap
        markers where not — the source model degrades explicitly, never silently.
  - [x] Success: resolution rule, citation/quoting rule, and explicit fallback present.

- [x] **3.3 Author the filesystem-signals source** — Effort: 2/5
  - [x] The closed checklist, verbatim in substance from the design (lines 158–164): test tree
        ...
        is not probed.
  - [x] Reported as observations with paths. **Absence of a signal is an observation, not a
        gap** — state this with the design's example ("no test tree observed at the repo root").
  - [x] Success: closed checklist matches the design item-for-item; observation-not-gap rule
        stated.

- [x] **3.4 Verify the sources against this repo** — Effort: 1/5
  - [x] Execute the graph selections as the skill states them (fields return non-empty on this
  ...
  - [x] Success: all three sources produce the design's Verified-facts values as written.
  - [x] Commit: `feat: add three-source extraction model to concept flow`

## Task 4: Per-section mapping

- [x] **4.1 Add the per-section mapping table** — Effort: 2/5
  - [x] Reproduce the design's seven-row table (lines 175–183): section, sources in order, human
        role. Rows are binding; do not paraphrase source names. Order is the concept guide's own
        section order.
  - [x] State the dropped-topics rule with it: why-now, audience-evolution, and
  ...
  - [x] State Solution Approach's coverage boundary: sourced from 362's coverage facts
  ...
  - [x] Success: table matches the design row-for-row; dropped-topics rule and coverage-boundary
        sourcing adjacent to it.

- [x] **4.2 Verify table fidelity** — Effort: 1/5
  - [x] Diff-check each row against the design; confirm no source renamed, dropped, or added, and
        that Target Users' human role reads "none — never asked".
  - [x] Success: row-for-row match confirmed.
  - [x] Commit: `feat: add per-section source mapping to concept flow`

## Task 5: Engagement interview and the single confirmation

- [x] **5.1 Author the engagement interview** — Effort: 2/5
  - [x] Exactly two questions, fixed text copied verbatim from the design (lines 204–210), asked
        once, as one block, after extraction and before drafting.
  - [x] Both skippable without argument or follow-up. A declined question produces a gap marker
  ...
  - [x] Answers land verbatim in User-Provided Concept; used twice, asked once — Q1 also feeds
        Problem & Motivation's engagement half, Q2 feeds Development Approach's constraints.
  - [x] State in the skill: an improvised or added question is a defect against Success
        Criterion 2, not a judgment call.
  - [x] Success: both questions verbatim; once-as-one-block timing, skip semantics, verbatim
        landing, dual use, and the defect rule all present.

- [x] **5.2 Author the single confirmation** — Effort: 2/5
  - [x] After drafting, before the file write: show the derived project description (assembled
        from `project.description` and the README lead) together with the graph's
        `lastAnalyzedAt`; ask confirm-or-correct.
  - [x] Three outcomes with their provenance records: confirmed → extracted-and-confirmed;
  ...
  - [x] State: this is the only confirmation in the flow; graph-derived structure is attributed,
        not confirmed section-by-section.
  - [x] Success: trigger point, all three outcomes, never-stalls rule, and only-confirmation
        statement present.

- [x] **5.3 Verify interview fidelity** — Effort: 1/5
  - [x] Diff the skill's question block against the design's — byte-identical wording.
        Confirm no third question, no follow-up prompt, and no confirmation besides 5.2's exists
        anywhere in the flow text.
  - [x] Success: byte-identical questions; interaction inventory is exactly two questions plus
        one confirmation.
  - [x] Commit: `feat: add engagement interview and single confirmation to concept flow`

## Task 6: User-Provided Concept contract and re-run semantics

- [x] **6.1 Author the write-time contract** — Effort: 2/5
  - [x] Check 1: `guide.ai-project.000-concept.md` readable at its expected path. Unreadable or
        absent → **stop**, naming the path; whole guide tree absent → name the setup step
        (`cf init` / `/cf:onboard`) instead of just the file.
  - [x] Check 2: the guide's document-structure block contains a section titled exactly
  ...
  - [x] State: neither failure is a gap marker — these mean the document cannot be correctly
        written at all.
  - [x] Write rules: verbatim (never summarized, reworded, or grammar-corrected); pre-existing
        section content survives untouched; new answers append below under a dated subheading.
  - [x] Success: both checks with their distinct stop messages, the not-a-gap statement, and
        both write rules present.

- [x] **6.2 Author re-run semantics** — Effort: 2/5
  - [x] No existing document → write it. Existing document → never overwrite: report it, offer
        augment or stop, **stop is the default**.
  - [x] Augment appends to User-Provided Concept per the preservation rule and fills only
  ...
  - [x] State the augment interaction: an augment run performs the flow's normal interview — the
  ...
  - [x] State: a human-edited concept is never rewritten from a graph.
  - [x] Success: default-stop rule, mechanical refillability test, and never-rewritten rule
        present.

- [x] **6.3 Verify the contract checks run** — Effort: 1/5
  - [x] Execute both checks against the real guide: path readable;
        `grep -c '^## User-Provided Concept'` returns 1.
  - [x] Success: both pass on this repo. (The failure path is exercised in Task 8.3 against a
        scratch copy — never the real guide.)
  - [x] Commit: `feat: add user-provided-concept contract and re-run semantics`

## Task 7: Governance, output conventions, provenance

- [x] **7.1 Record `[INFERRED]` governance for this flow** — Effort: 1/5
  - [x] The checkable rule, in substance from the design (lines 278–280): a sentence carries
        ...
        does not belong.
  - [x] Prose interacts by citation, not inference: a claim the README states is cited and
  ...
  - [x] Update the shared Gap markers section's pointer to this flow's `[INFERRED]` governance to
        point at the actual subsection (Implementation Notes, design line 448).
  - [x] Success: rule, citation-not-inference statement, confirmed-stays-marked rule, provenance
        listing, and updated pointer all present.

- [x] **7.2 Author output conventions** — Effort: 2/5
  - [x] Path: `project-documents/user/project-guides/000-concept.{project}.md`, `{project}` from
  ...
  - [x] Frontmatter: the design's block (lines 297–312) — `docType: concept`, `status:
        not_started`, and `model:` per 361's rule unchanged (real generating model id or an
        explicit stop, never a placeholder).
  - [x] Success: path rule with divergence handling; frontmatter block matches the design;
        `model:` rule referenced, not restated.

- [x] **7.3 Author the provenance block** — Effort: 2/5
  - [x] 361's shape with concept-specific content (design lines 317–328): Generated by; Source
  ...
  - [x] Success: all block lines present; outcome set complete, none invented.

- [x] **7.4 Fix the 362 cross-reference sentence** — Effort: 1/5
  - [x] Edit the comprehension flow's sentence "The concept flow and the initiative-candidates
        flow are slices 363 and 364" to reflect that the concept flow now exists (364 remains
        future).
  - [x] Success: sentence accurate; no other comprehension-flow text changed.
  - [x] Commit: `feat: add inferred governance, output conventions, and provenance to concept flow`
...
- [x] **8.1 Happy path (PM live)** — Effort: 3/5
  - [x] Run the concept flow against squadron. PM answers both engagement questions; when shown
        the derived description, the PM corrects it (the graph's is measurably stale — this
        exercises the correction outcome).
  - [x] Confirm in the written `000-concept.squadron.md`, per design step 3 (lines 397–411):
  ...
  - [x] Dropped-topics absence check (SC4): search the written document for why-now,
  ...
  - [x] `[INFERRED]` audit (SC10): enumerate every `[INFERRED]` sentence in the document; each
        ...
        defect.
  - [x] `cf validate frontmatter` passes on the file; `model:` is a real model id.
  - [x] Success: every listed check passes on the written document, including both audits.

- [x] **8.2 Decline path (scratch copy of the tree)** — Effort: 2/5
  - [x] On a scratch copy, decline both questions. Document still written; gap markers where
        Q1/Q2 content would land; provenance lists both as declined; no section holds prose
        without a source.
  - [x] Success: all decline-path checks pass; the real tree is untouched.

- [x] **8.3 Contract and precondition failures (scratch paths only)** — Effort: 2/5
  - [x] Safe mechanism, same for every case: copy the guide (or build an empty directory tree)
  ...
  - [x] Case 1 — renamed section: scratch copy with the `## User-Provided Concept` heading
        renamed. Loud stop naming the guide, the expected title, and that the layout appears
        changed upstream.
  - [x] Case 2 — guide file absent/unreadable: scratch tree with the guide file removed. Loud
        stop naming the missing path.
  - [x] Case 3 — guide tree absent: scratch tree with no `ai-project-guide/` at all. Loud stop
        naming the setup step (`cf init` / `/cf:onboard`), not just the file.
  - [x] In every case, nothing is written.
  - [x] Success: all three SC7 failure cases produce their distinct loud stops; no file written;
        repo untouched after each.

- [x] **8.4 Re-run semantics** — Effort: 2/5
  - [x] Default stop, against the **real** step-8.1 document: re-run the flow; it reports the
  ...
  - [x] Refillability, against a **scratch** copy of the 8.1 document prepared with one Refined
        ...
        the UPC append).
  - [x] Success: default stop proven non-mutating on the real document; scratch augment refills
        exactly the two prepared sections, appends under a dated subheading, and touches nothing
        else.

- [x] **8.5 Flow selection live** — Effort: 1/5
  - [x] No argument and `comprehension` route to the comprehension flow (unchanged behavior);
        `candidates` stops as unrecognized.
  - [x] Success: all three routes behave as authored.

- [x] **8.6 Discipline and scope checks (preliminary — final scope check is in 9.3)** — Effort: 1/5
  - [x] From the 8.1 session: no graph read without a `jq` field selection; no `function`/`class`
        node read. `git status` on `.understand-anything/` clean.
  - [x] `git diff --name-only main` shows only `commands/analysis/understand.md`, the generated
  ...
  - [x] Project regression guards (convention, not slice-derived criteria — the slice adds no
        Python): `uv run ruff format --check .` passes; `uv run pytest tests/skills/` passes
        (pack install path undisturbed).
  - [x] Success: all checks clean; changed-file set as stated.
  - [x] Commit: `feat: add generated concept document and walkthrough fixes`
...
- [x] **9.1 Update task and slice status** — Effort: 1/5
  - [x] All checkboxes above verified against walkthrough evidence; dropped/skipped items (if
  ...
  - [x] Success: statuses consistent across task file, slice design, and slice plan.

- [x] **9.2 DEVLOG entry** — Effort: 1/5
  - [x] Write the slice-completion entry per `prompt.ai-project.system.md` → Session State
...
- [x] **9.3 Final commit and hand-off** — Effort: 1/5
  - [x] `uv run ruff format .` before committing (project rule), commit remaining changes,
        confirm clean tree.
  - [x] Final scope check, after all close-out edits: `git diff --name-only main` shows only
  ...
  - [x] Success: branch complete and clean; final scope check passes; PM notified; no merge
        performed.
