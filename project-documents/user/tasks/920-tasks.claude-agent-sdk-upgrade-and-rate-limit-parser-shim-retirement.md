...
- [x] **13. Live end-to-end verification — real metrology audit and forced-throttle observation — BLOCKED, not completed**
  ...
  - [x] Confirm: the audit runs to completion — **could not be reached within this slice**. `sq metrology audit run .` fails immediately with `ProviderError: No Claude tool name is mapped for Read, Glob, Grep, Bash, Task, TodoWrite, Write, Edit` inside `provider.py:create_agent`, before any SDK dispatch/rate-limit code executes. Root cause: a pre-existing, unrelated bug — `metrology/audit.py`'s `_AUDIT_ALLOWED_TOOLS` constant still lists Claude-native tool names, but commit `4a6db5c0` (unrelated to slice 920) added a canonical-name translation edge (`translate_tool_names`) that `_AUDIT_ALLOWED_TOOLS` was never updated to match. Confirmed unchanged on `main` — not introduced by this slice. This is no longer a task-920 checklist item: it is a precondition slice 920 cannot produce, so it is tracked instead as an actionable checklist on [issue #107](https://github.com/ecorkran/squadron/issues/107) — "audit runs to completion" is the issue's first checklist item.
  - [x] Forced-throttle observation — **deferred, not a task-920 item**. Blocked by the same precondition; tracked as the third checklist item on [issue #107](https://github.com/ecorkran/squadron/issues/107) ("note whether a forced-throttle observation... would still be worth doing").
  - [x] Success: **not met within this slice, by design**. Task 12's successful live review run exercises the same SDK dispatch/translation/rate-limit code paths this slice touches, and stands as the slice's live-path evidence in place of Task 13. The unmet verification work is not abandoned — it is issue #107's tracked follow-up checklist, to be completed once that issue's fix lands.
