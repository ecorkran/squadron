...
- [ ] **1.1 Create the package and limits module** — Effort: 1/5
  - [ ] Create directory `src/squadron/tools/`.
  - [ ] Create `src/squadron/tools/limits.py` with exactly three module-level constants and a
  ...
  - [ ] Do not add any other value to this module. Do not add configuration plumbing —
        slice 266 owns the configuration surface.
  - [ ] Success: `python -c "from squadron.tools import limits; print(limits.MAX_READ_BYTES)"`
        prints `256000`.

- [ ] **1.2 Create `errors.py`** — Effort: 1/5
  - [ ] Define `ToolNotRegisteredError(Exception)` with a docstring stating it signals a *caller
        configuration* error (an unknown name passed to `materialize`), not a model error.
  - [ ] No other exception types. Executors return `ToolResult(is_error=True)`; they do not raise
        to the caller.
  - [ ] Success: importable; `issubclass(ToolNotRegisteredError, Exception)` is true.

- [ ] **1.3 Create `models.py` with the descriptor and result types** — Effort: 2/5
  - [ ] Define the two type aliases first, in this order (the factory alias references the
        executor alias):
        - `ToolExecutor = Callable[[dict[str, object]], Awaitable[ToolResult]]`
        - `ToolFactory = Callable[[Path], ToolExecutor]`
  - [ ] Define `@dataclass(frozen=True) class ToolResult` with fields `content: str` and
        `is_error: bool = False`.
  - [ ] Define `@dataclass(frozen=True) class ToolDescriptor` with fields `name: str`,
        `description: str`, `parameters: dict[str, object]`, `factory: ToolFactory`.
  - [ ] Docstring on `parameters` states it is a JSON Schema object matching OpenAI's
        `tools[].function.parameters` shape.
  - [ ] Docstring on `factory` states it receives an **already-resolved** `cwd` and returns a
        closure-bound executor. `cwd` never appears in `parameters` — the model cannot supply it.
  - [ ] Do **not** define a `Protocol`. Design decision D2: one shape, no polymorphism, a Protocol
        with a single implementer is unjustified complexity.
  - [ ] Use `from __future__ import annotations` and put `Callable`/`Awaitable` imports under
        `collections.abc` (matching the rest of the codebase).
  - [ ] Success: `ruff check src/squadron/tools/` and `pyright` both clean for these files;
        `ToolResult("x").is_error is False`; the dataclass is frozen (assignment raises).

- [ ] **1.4 Test the pure types** — Effort: 1/5
  - [ ] Create `tests/tools/__init__.py` and `tests/tools/test_models.py`.
  - [ ] Assert `ToolResult` defaults `is_error` to `False` and is frozen
        (`pytest.raises(dataclasses.FrozenInstanceError)` on attribute assignment).
  - [ ] Assert `ToolDescriptor` is frozen and stores all four fields verbatim.
  - [ ] Assert a descriptor's `factory` invoked with a `Path` returns a callable
...
- [ ] **2.1 Implement `registry.py`** — Effort: 2/5
  - [ ] Module-level `_REGISTRY: dict[str, ToolDescriptor] = {}`. Free functions, not a class —
        mirroring [providers/registry.py](src/squadron/providers/registry.py).
  - [ ] `register(descriptor: ToolDescriptor) -> None` — stores under `descriptor.name`. If the
  ...
  - [ ] `lookup(name: str) -> ToolDescriptor | None` — returns `None` for unknown names. Does not
        raise.
  - [ ] `list_tools() -> list[str]` — returns registered names.
  - [ ] `materialize(names: Sequence[str], cwd: str | Path) -> dict[str, ToolExecutor]`:
  ...
  - [ ] Do not add an `ensure_tools_loaded()` indirection (design D5) — there is one built-in
        module, and registration happens at package import.
  - [ ] Success: `ruff check` and `pyright` clean; functions annotated and documented.

- [ ] **2.2 Test the registry** — Effort: 2/5
  - [ ] Create `tests/tools/test_registry.py`.
  - [ ] **Isolate registry state.** The registry is module-level and the built-ins register at
  ...
  - [ ] Assert `register` then `lookup` returns the same descriptor object.
  - [ ] Assert `register` with an already-registered name raises `ValueError` and the message
        names the tool.
  - [ ] Assert `lookup("nope")` returns `None` (does not raise).
  - [ ] Assert `list_tools()` includes a freshly registered name.
  - [ ] Assert `materialize` returns a dict keyed by the requested names, whose values are
        callables.
  - [ ] Assert `materialize` with an unknown name raises `ToolNotRegisteredError`, and the message
        contains both the unknown name and at least one registered name.
  - [ ] Assert `cwd` is resolved once and passed resolved: register a probe descriptor whose
...
- [ ] **3.1 Implement the path jail helper** — Effort: 2/5
  - [ ] In `builtin.py`, add a module-private helper that takes the resolved `cwd` and the
        model-supplied `path` string and returns the resolved target `Path`, or signals rejection.
  - [ ] Rule: `candidate = (cwd / path).resolve(strict=False)`; accept only if
  ...
  - [ ] The rejection path must produce a `ToolResult(is_error=True)` whose content names the
...
- [ ] **3.2 Implement the shared executor wrapper and logging contract** — Effort: 2/5
  ...
  - [ ] Logging levels (design §Observability, review F003) — this is a **contract**, not a
        suggestion:
  ...
  - [ ] Do not elevate the INFO cases to WARNING. They are routine model-probing outcomes the
...
- [ ] **4.1 Implement the `read_file` descriptor and executor** — Effort: 2/5
  - [ ] Parameters JSON Schema: object with one required property `path` (string), described for
        the model as relative to the working directory. `cwd` must **not** appear.
  - [ ] Factory binds the resolved `cwd`; executor runs the jail check, then performs the read.
  - [ ] Read text as UTF-8 with `errors="replace"`. Wrap the blocking filesystem work in
        `asyncio.to_thread` — files may be large and must never block the event loop.
  - [ ] Truncate content beyond `limits.MAX_READ_BYTES`, appending a **visible trailing marker
  ...
  - [ ] Error results with specific messages for: path escape (WARNING), missing file, path is a
        directory, permission denied (INFO each).
  - [ ] Register the descriptor at module scope via `register(...)`.
  - [ ] Success: descriptor present in `list_tools()`; executor is `async`.

- [ ] **4.2 Test `read_file`** — Effort: 2/5
  - [ ] Create `tests/tools/test_read_file.py`. Use `tmp_path` as the jail root throughout.
  - [ ] Happy path: write a file under `tmp_path`, read it via the materialized executor, assert
        content matches and `is_error is False`.
  - [ ] Absolute path *inside* the jail succeeds (proving the jail check does not reject all
        absolute paths).
  - [ ] Jail rejections, each asserting `is_error is True` **and** that the message names the
        rejected path:
  ...
  - [ ] Truncation: monkeypatch `limits.MAX_READ_BYTES` to a small value (or write a file larger
  ...
  - [ ] Missing file and directory-as-path each return `is_error=True` with distinct messages.
  - [ ] `caplog` assertions: the jail rejection logs at **WARNING**; the missing-file case logs at
...
- [ ] **5.1 Implement the `write_file` descriptor and executor** — Effort: 2/5
  - [ ] Parameters JSON Schema: object with required `path` (string) and `content` (string).
  - [ ] Same jail check as `read_file`, via the shared helper from Task 3.1.
  - [ ] Create parent directories (`mkdir(parents=True, exist_ok=True)`). The **parent directory
        path is jail-checked before creation** — never create directories outside the jail.
  - [ ] Existing files are overwritten. The result content states **created vs. overwritten** and
  ...
  - [ ] Blocking filesystem work wrapped in `asyncio.to_thread`.
  - [ ] Error results: path escape (WARNING), permission denied, path is an existing directory
        (INFO each).
  - [ ] Register the descriptor at module scope.
  - [ ] Success: descriptor present in `list_tools()`; executor is `async`.

- [ ] **5.2 Test `write_file`** — Effort: 2/5
  - [ ] Create `tests/tools/test_write_file.py`, `tmp_path` as jail root.
  - [ ] Create-new: write to `notes/a.txt` (nested, parent does not exist), assert the file exists
  ...
  - [ ] Overwrite: write the same path again with different content; assert the result says
        overwritten and the byte count matches the new content.
  - [ ] Jail rejections mirroring Task 4.2 (relative traversal, absolute-outside, symlink-outside),
...
- [ ] **6.1 Implement the `bash` descriptor and executor** — Effort: 3/5
  - [ ] Parameters JSON Schema: object with one required property `command` (string).
  - [ ] Spawn with `asyncio.create_subprocess_shell(command, cwd=cwd, stdout=PIPE, stderr=PIPE,
        start_new_session=True)`. `start_new_session=True` is required so the timeout path can
        kill the whole process group.
  - [ ] Await `proc.communicate()` inside `asyncio.wait_for(..., timeout=limits.BASH_TIMEOUT_S)`.
  - [ ] **Timeout path:** on `TimeoutError`, kill the process group via
  ...
  - [ ] Decode stdout and stderr with `errors="replace"`. Result content labels each stream, and
        truncates each to `limits.MAX_OUTPUT_BYTES` with a visible marker.
  - [ ] Non-zero exit → `is_error=True` with the exit code **and the captured output** (the model
        needs the output to react). Log at **INFO**.
  - [ ] CWD is the only boundary at this stage. Do **not** add network denial, env scrubbing, or
        process isolation — arch-documented future work, explicitly out of scope.
  - [ ] Register the descriptor at module scope.
  - [ ] Success: descriptor present in `list_tools()`; executor is `async`.

- [ ] **6.2 Test `bash`** — Effort: 3/5
  - [ ] Create `tests/tools/test_bash.py`, `tmp_path` as the working directory.
  - [ ] Happy path: run a command that writes to stdout; assert the output appears in the result,
        labeled, with `is_error is False`.
  - [ ] Stderr capture: run a command writing to stderr; assert the stderr content appears,
        labeled.
  - [ ] CWD anchoring: create a file in `tmp_path`, run `ls` (or equivalent), assert the filename
        appears — proving the subprocess runs in the jail root and not the repo root.
  - [ ] Non-zero exit: run a command exiting non-zero; assert `is_error is True`, the exit code is
        in the message, and captured output is still present.
  - [ ] Truncation: monkeypatch `limits.MAX_OUTPUT_BYTES` to a small value, produce more output
        than that, assert the result is shortened and contains the marker.
  - [ ] **Timeout: monkeypatch `limits.BASH_TIMEOUT_S` to a small value** (well under a second is
  ...
  - [ ] `caplog`: the timeout logs at **WARNING**; the non-zero-exit case logs at **INFO**.
...
- [ ] **7.1 Write `tools/__init__.py`** — Effort: 1/5
  - [ ] Import `builtin` so registration happens at package import (design D5), marking it
  ...
  - [ ] Re-export the public API: `ToolDescriptor`, `ToolResult`, `ToolExecutor`, `ToolFactory`,
        `ToolNotRegisteredError`, `register`, `lookup`, `materialize`, `list_tools`.
  - [ ] Define `__all__` listing exactly those names.
  - [ ] Importing the package must have **no side effects other than registration** — no logging
        configuration, no filesystem access, no environment reads.
  - [ ] Success: `python -c "from squadron import tools; print(tools.list_tools())"` prints
        `['read_file', 'write_file', 'bash']`.

- [ ] **7.2 Test the import surface** — Effort: 1/5
  - [ ] Create `tests/tools/test_package.py`.
  - [ ] Assert `set(tools.list_tools()) == {"read_file", "write_file", "bash"}` — exactly these
        three, no more.
...
- [ ] **8.1 Confirm limits are not scattered** — Effort: 1/5
  - [ ] Run `grep -rn "256_000\|256000\|64_000\|64000\|120\.0" src/squadron/tools/` and confirm
        every hit is inside `limits.py`. Any literal elsewhere is a defect — reference the
        constant instead.
  - [ ] Success: the only hits are in `limits.py`.

- [ ] **8.2 Confirm nothing consumes tools yet** — Effort: 1/5
  - [ ] Run `grep -rn "squadron.tools\|squadron import tools" src/` and confirm the only hits are
        inside `src/squadron/tools/` itself.
  - [ ] Run `git status` and confirm no file outside `src/squadron/tools/` and `tests/tools/` is
  ...
  - [ ] Success: both checks clean.

- [ ] **8.3 Run the full gate** — Effort: 1/5
  - [ ] `.venv/bin/ruff format .` then `.venv/bin/ruff check .` — both clean.
  - [ ] `.venv/bin/pyright` — no new errors under `src/squadron/tools/` (strict mode applies to
        `src`).
  - [ ] `.venv/bin/pytest tests/tools/ -q` — new suite passes.
  - [ ] `.venv/bin/pytest -q` — **full suite**, no regression anywhere else.
  - [ ] Success: all four clean. Report actual output; do not summarize a failure as a pass.

- [ ] **8.4 Manual walkthrough** — Effort: 1/5
  - [ ] Registry demo:
        `python -c "from squadron import tools; print(tools.list_tools())"` →
        `['read_file', 'write_file', 'bash']`.
  - [ ] Jail demo — run the scratch-tree script from the design document's Verification
  ...
  - [ ] Record the actual output in the DEVLOG entry (Task 8.6). If any step deviates from the
        expected result, fix the code — do not adjust the expectation.

- [ ] **8.5 Refine the design's Verification Walkthrough** — Effort: 1/5
  - [ ] The design marks that section "Draft — refined at Phase 6 completion." Replace the draft
...
- [ ] **8.6 DEVLOG and commit** — Effort: 1/5
  - [ ] Append a DEVLOG entry to `DEVLOG.md` at the repo root (**not**
...
- [ ] 1. `ToolDescriptor` / `ToolResult` / `ToolExecutor` exist as specified (Task 1.3).
- [ ] 2. Registry: duplicate `register` → `ValueError`; `lookup` → `None` for unknown;
...
- [ ] 6. `write_file` creates nested dirs, overwrites, and reports created/overwritten + bytes
      (Task 5.2).
- [ ] 7. `bash` labels stdout/stderr, non-zero exit → error with code, timeout kills the process
      group and reports, oversized output truncated visibly (Task 6.2).
- [ ] 8. All limits are named constants in `limits.py`; no limit literal elsewhere (Task 8.1).
- [ ] 9. Every failure mode has at least one test, including `caplog` assertions that jail
