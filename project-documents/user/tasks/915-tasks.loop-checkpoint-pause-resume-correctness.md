...
- [x] Effort: 1/5
- [x] In [state.py](src/squadron/pipeline/state.py), define a module-level
      `_RESUMABLE_STATUSES` built from `ExecutionStatus` members — **not** string
      literals — containing `PAUSED` and `FAILED`.
- [x] `StepState.status` is a `str` holding the serialized `.value`, so the set
      must hold `.value` strings produced from the enum members at that one site.
- [x] Change `first_unfinished_step` ([state.py:441](src/squadron/pipeline/state.py#L441))
      so `completed` excludes any `StepState` whose `status` is in
      `_RESUMABLE_STATUSES`.
- [x] Add a short comment stating why `FAILED` is included: the top-level walk
...
- [x] Effort: 1/5
- [x] Add tests to [tests/pipeline/test_state.py](tests/pipeline/test_state.py)
      following that file's existing state-construction pattern.
- [x] Case: step recorded `PAUSED` → predicate returns that step's name, not its
      successor.
- [x] Case: step recorded `FAILED` → predicate returns that step's name.
- [x] Case: all steps recorded `COMPLETED` → predicate returns `None`
      (guards the "All steps already completed. Nothing to resume." regression
      named in the design's Success Criteria).
- [x] Case: a paused step followed by later completed steps → predicate returns
...
- [x] Effort: 1/5
- [x] Add `resume_iteration_for(run_id: str, step_name: str) -> int` to
      `StateManager` in [state.py](src/squadron/pipeline/state.py).
- [x] Return the recorded `iteration` for the named step from
...
- [x] If a step name appears more than once in `completed_steps`, return the
      **last** occurrence's iteration — that is the most recent record.
- [x] Docstring notes this is the first reader of `StepState.iteration`
...
- [x] Case: a paused loop step recorded at iteration 2 → returns `2`.
- [x] Case: unknown step name → returns `0`.
- [x] Case: a non-loop step (recorded iteration `0`) → returns `0`.
- [x] Case: the same step name appearing twice in `completed_steps` → returns the
...
- [x] Effort: 1/5
- [x] Read [run.py:653](src/squadron/cli/commands/run.py#L653) (finalize-on-`None`)
      and [run.py:799](src/squadron/cli/commands/run.py#L799) (next-step display).
- [x] Confirm the 653 path does **not** now finalize a run as `COMPLETED` that
      ...
      merely assumed.
- [x] Confirm the 799 display path reports the paused step as "next" rather than
      its successor, and that its surrounding copy still reads correctly.
- [x] If either path needs an adjustment, make the minimal one and note it here.
...
- [x] Effort: 2/5
- [x] Add a test covering the 653 finalize path: a run state containing a paused
      step must not be finalized as `COMPLETED`.
- [x] Add a test covering the 799 display path: the reported next step is the
      paused step.
- [x] Place these with the existing CLI-level run tests, following current file
      organization.
...
- [x] Effort: 1/5
- [x] Run `ruff format .`, then `ruff check .`, then strict `pyright` — all clean.
- [x] Run the full test suite; no regressions.
- [x] Commit Part A on its own (design Risk Assessment: a bisect must separate
      "resume returns to paused steps" from "loops re-enter at a round").
- [x] Suggested message: `fix: return resume to paused and failed steps`
...
- [x] Effort: 1/5
- [x] At the inner-`PAUSED` short-circuit in `_execute_loop_body`
      ([executor.py:1283-1290](src/squadron/pipeline/executor.py#L1283-L1290)),
      log at WARNING before returning.
- [x] The message must name: pipeline name, loop step name, the paused iteration,
      and the number of rounds not run (`loop_config.max - iteration`).
- [x] Use the module logger and lazy `%s` formatting, matching the file's existing
      logging style.
...
- [x] Effort: 1/5
- [x] `_execute_loop_step` ([executor.py:1077](src/squadron/pipeline/executor.py#L1077))
...
- [x] Do not duplicate the message string at two sites — extract one small helper
...
- [x] Add tests to
      ...
      using `caplog`.
- [x] Case: multi-step body pauses at round 1 of `max: 3` → exactly one WARNING,
      containing the step name, `1`, and a rounds-not-run count of `2`.
- [x] Case: single-step body pauses → the same WARNING is emitted.
- [x] Case: a loop that converges normally emits **no** such WARNING.
...
- [x] Effort: 1/5
- [x] `ruff format .`, `ruff check .`, strict `pyright` clean; suite green.
- [x] Commit. Suggested message: `fix: warn when a loop abandons rounds on pause`
...
- [x] Effort: 2/5
- [x] Add `start_iteration: int = 1` to `_execute_loop_body`
...
- [x] Change **both** hardcoded ranges — [executor.py:1114](src/squadron/pipeline/executor.py#L1114)
...
- [x] Nothing inside a round changes. `prior_iteration_step_outputs` starts empty
...
- [x] Effort: 2/5
- [x] In [tests/pipeline/test_executor_loop_body.py](tests/pipeline/test_executor_loop_body.py):
      `start_iteration=2`, `max: 3` → exactly two rounds execute, numbered 2 and 3.
- [x] `start_iteration=1` (default) → unchanged from current behavior.
- [x] `start_iteration == max` → exactly one round runs.
- [x] `start_iteration > max` → zero rounds run. This is only reachable from
...
- [x] Assert both the returned status and the WARNING for that case.
- [x] Cover the single-step (`_execute_loop_step`) path as well as the multi-step
      one.
...
- [x] Effort: 2/5
- [x] Add `start_from_iteration: int = 0` to `execute_pipeline`
      ([executor.py:497](src/squadron/pipeline/executor.py#L497)) — additive with a
      non-breaking default.
- [x] It applies to the `start_from` step **only**; every other step uses the
      normal start.
- [x] When the target step is a loop: clamp to `>= 1` before passing as
      `start_iteration`.
- [x] When the target step is **not** a loop: ignore it and log at DEBUG that it
      was ignored, naming the step (design D3).
- [x] Emit the resume-time INFO when re-entering a loop at iteration > 1, naming
...
- [x] Case: `start_from` a loop step with `start_from_iteration=2` → the loop runs
      rounds 2+ and an INFO naming step and round is emitted.
- [x] Case: `start_from` a non-loop step with a non-zero iteration → behavior
      identical to `start_from_iteration=0`, and a DEBUG "ignored" line is logged.
- [x] Case: `start_from_iteration=0` on a loop step → clamps to round 1.
- [x] Case: steps before `start_from` are still skipped, unchanged.
...
- [x] Effort: 2/5
- [x] Explicit `--resume` ([run.py:1102](src/squadron/cli/commands/run.py#L1102)):
...
- [x] Implicit paused-run detection ([run.py:1159](src/squadron/cli/commands/run.py#L1159)):
      the identical treatment.
- [x] Do not duplicate the lookup logic between the two paths — both read the
...
- [x] Effort: 3/5
- [x] Add a CLI-level test: a pipeline whose loop body is `[dispatch, review]`
      with `checkpoint: on-concerns` and `max: 3`, against a review returning
      CONCERNS on round 1.
- [x] Assert the run pauses at the loop step and the persisted `StepState` records
      `status="paused"` with the correct `iteration`.
- [x] Assert that resume **re-enters the loop** rather than proceeding to the next
      step.
- [x] Assert the `max:` counting rule: a loop paused at round 2 of 3 resumes at
      round 2 and runs at most rounds 2–3 — resume neither restarts the count nor
      grants extra rounds (design Success Criteria).
- [x] Assert the clean-run regression: a fully completed run still reports
...
- [x] Effort: 1/5
- [x] `ruff format .`, `ruff check .`, strict `pyright` clean; full suite green.
- [x] Commit. Suggested message: `fix: resume re-enters a paused loop at its round`
...
- [x] Effort: 3/5
- [x] Run all **eight** steps of the Verification Walkthrough in the slice design
...
- [x] Step 1 requires reproducing the bug **before** the fix. If Part A is already
      ...
      bug.
- [x] Step 6 must be confirmed against the **persisted state file**, not console
      output alone — it is the only check in this slice covering a changed stored
      outcome.
- [x] Correct the walkthrough text in the design document wherever reality differs
      from the draft. Do not leave a walkthrough that was never run.
- [x] Verify the design's corrected claims still hold against the code as
...
- [x] Effort: 1/5
- [x] Document that a checkpoint pausing inside a loop body resumes **into** that
      loop at the paused round.
- [x] Document the `max:` counting rule: rounds are counted per loop, not per
      invocation — resume does not restart the count and does not grant extra rounds.
- [x] Document the round contract on re-entry: `prior_iteration_step_outputs` is
      empty for a resumed round (design D3), so a resumed round cannot read the
      previous round's outputs.
- [x] Document the known limitation: `each:` / `fan_out:` steps resume by
...
- [x] Effort: 1/5
- [x] File a GitHub issue for per-branch re-entry of paused `each:` / `fan_out:`
      ...
      exists).
- [x] Note that slice 915 Part A improved these from *silently skipped* to
      *restarted from the top*, and that re-entry needs a per-branch completion
      record — a larger data-model change.
- [x] Reference issue #48 and this slice.
...
- [x] Effort: 1/5
- [x] Write a DEVLOG entry per `prompt.ai-project.system.md` § Session State Summary.
- [x] Add a short user-facing CHANGELOG bullet (technical detail belongs in DEVLOG).
- [x] Close [issue #48](https://github.com/ecorkran/squadron/issues/48) with a
      comment naming the fixing commits.
- [x] Mark slice 915 complete in the slice design and in
      `900-slices.maintenance-and-refactoring.md`.
- [x] Merge the slice branch into the integration target
