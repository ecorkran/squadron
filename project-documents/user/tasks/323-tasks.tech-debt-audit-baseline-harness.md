...
- [x] **The variance campaign** — partially complete; see the deferred item below
  ...
  - [x] **Deferred, not dropped: `context-forge`, `squadron`.** Both are larger than `migratory`. Two projects spanning a 14x size difference already establish both that a floor exists and that it is size-dependent; the remaining two would refine the size relationship rather than establish it. Carried to Future Work
