...
- [ ] Read `src/squadron/pipeline/sdk_session.py` — confirm
  `capture_summary()` signature and that nothing else needs to change
  on the SDK side
- [ ] Read `src/squadron/review/review_client.py` — re-read
...
- [ ] Create new file `src/squadron/pipeline/summary_oneshot.py` with
  module docstring and `from __future__ import annotations`
- [ ] Add `_is_sdk_profile(profile: str | None) -> bool`:
  - Returns `True` when `profile is None` (unannotated alias)
  - Returns `True` when `profile == ProfileName.SDK.value`
  - Returns `False` otherwise
- [ ] Import `ProfileName` from `squadron.providers.base`
- [ ] Function is module-public (no underscore) so it can be imported
  by both `summary.py` and `prompt_renderer.py` without going through
  a private name
- [ ] Rename to `is_sdk_profile` (drop the leading underscore — it's
  cross-module)
  - [ ] File created at correct path
  - [ ] `ProfileName.SDK.value` is the only string compared (no raw
    `"sdk"` literal)
  - [ ] Function has a one-line docstring naming the contract:
    "None and 'sdk' both route through the SDK session"

### T3: Test `is_sdk_profile()`

- [ ] Create `tests/pipeline/test_summary_oneshot.py` (or reuse if
  someone else opened it first)
- [ ] Parametrized test covering: `None` → `True`, `"sdk"` → `True`,
  `"openrouter"` → `False`, `"openai"` → `False`, `"gemini"` → `False`,
  `"local"` → `False`, `"openai-oauth"` → `False`, `"unknown-future"`
  → `False`
- [ ] Run: `uv run pytest tests/pipeline/test_summary_oneshot.py -v`
  - [ ] All cases pass
  - [ ] Test imports `is_sdk_profile` from
...
- [ ] Add async function with signature:
  ```python
  ...
  ```
- [ ] Body mirrors `run_review_with_profile()` lines 70–149, with
...
- [ ] Iterate `agent.handle_message(message)` and accumulate
...
- [ ] Wrap agent lifecycle in `try` / `finally` with
  `await agent.shutdown()` in `finally`
- [ ] Return the concatenated string
- [ ] Imports at top of `summary_oneshot.py`:
  ...
  - [ ] No review-specific code paths leaked in (no rules, no
    structured output, no parsing)
  - [ ] `agent.shutdown()` is in `finally` so it always runs

### T5: Test `capture_summary_via_profile()` with a stub provider

- [ ] In `tests/pipeline/test_summary_oneshot.py`, add tests using a
  fake provider that records the message it received and returns a
  canned response
- [ ] Test: happy path — fake provider returns one
  `AgentMessage(content="SUMMARY OUTPUT")`; result equals
  `"SUMMARY OUTPUT"`
- [ ] Test: multi-chunk response — fake provider yields two messages;
  result is the concatenation
- [ ] Test: SDK_RESULT_TYPE duplication is filtered — fake provider
...
- [ ] Test: agent.shutdown() is called even when handle_message raises
  — fake provider raises mid-iteration; assert shutdown was called
- [ ] Use a registered fake profile + provider for the duration of the
...
- [ ] Run: `uv run pytest tests/pipeline/test_summary_oneshot.py -v`
  ...
  - [ ] Fake provider does not leak between tests

### T6: Update `_execute_summary()` to branch on profile

- [ ] In `src/squadron/pipeline/actions/summary.py` `_execute_summary()`:
  - [ ] Capture both elements from the resolver:
  ...
  - [ ] When `summary_model_alias is None`, set `profile = None` and
    `model_id = None` (the resolver isn't called; the SDK fallback uses
    the session's current model)
- [ ] Replace the unconditional `context.sdk_session is None` early
  return with a more precise guard:
  - [ ] If any emit destination has `kind is EmitKind.ROTATE` AND
  ...
  - [ ] Otherwise, proceed (non-SDK profile + non-rotate emit can run
    without a session)
- [ ] Add validation: if `is_sdk_profile(profile) is False` AND any
...
- [ ] Capture the summary by branching:
  ```python
  ...
  ```
- [ ] Wrap both branches in the existing `try` / `except Exception`
  that produces the failure `ActionResult` (preserve current error
  shape)
- [ ] Imports added at top of `summary.py`:
  - `from squadron.pipeline.summary_oneshot import capture_summary_via_profile, is_sdk_profile`
  - [ ] SDK fallback path is byte-identical for SDK-profile models
  - [ ] Non-SDK path goes through the new helper, never touches
    `set_model`
  - [ ] Rotate-validation error is raised before any provider call

### T7: Test `_execute_summary()` profile branching

- [ ] In `tests/pipeline/actions/test_summary.py`:
- [ ] **Update existing test** `test_execute_summary_no_sdk_session_returns_failure`:
...
- [ ] **Add** `test_execute_summary_routes_non_sdk_profile_via_oneshot`:
...
- [ ] **Add** `test_execute_summary_non_sdk_profile_with_rotate_fails`:
...
- [ ] **Add** `test_execute_summary_sdk_profile_path_unchanged`:
...
- [ ] Document the chosen shape inline in this task file under T8
  before proceeding to T9 (so T9 has a stable target)
- [ ] If a different shape is preferred, replace this and the
  following implementation tasks accordingly and note the divergence in
  the slice doc's OQ1 section
  - [ ] OQ1 resolution recorded
  - [ ] CLI surface fully specified before any code is written

### T9: Implement `sq summary-run` hidden subcommand

- [ ] Locate the appropriate Typer module (likely
...
- [ ] Add `summary_run` function with Typer signature matching T8:
...
- [ ] Register the command in the CLI app, hidden from `--help`
  (mirror the registration of `sq _summary-instructions`)
- [ ] Add error handling: catch and re-raise as `typer.Exit(code=1)`
  with a stderr message for: bad `param` format, missing template,
  unknown profile, provider failure
  - [ ] Command is reachable as `sq summary-run` (or `sq _summary-run`,
    matching the project's existing hidden-command convention — confirm
    with T1)
  - [ ] Hidden from `--help`
  - [ ] Returns nonzero on error with descriptive stderr

### T10: Test `sq summary-run` subcommand

- [ ] In `tests/cli/commands/test_summary_run.py` (new file):
- [ ] Test: happy path — invoke via Typer test runner with mocked
  `capture_summary_via_profile` returning canned text; assert stdout
  contains the text and exit code is 0
- [ ] Test: bad `--param` format (no `=`) → exit 1 with stderr
  mentioning the bad value
- [ ] Test: missing template → exit 1 with stderr mentioning the
  template name
- [ ] Test: provider raises → exit 1 with stderr mentioning the error
- [ ] Run: `uv run pytest tests/cli/commands/test_summary_run.py -v`
  - [ ] All tests pass
  - [ ] No test relies on a real network call

### T11: Update `_render_summary()` in `prompt_renderer.py`

- [ ] In `src/squadron/pipeline/prompt_renderer.py` `_render_summary()`:
  - [ ] When `model_raw is not None`, capture both elements from the
  ...
  - [ ] Quote params correctly so they survive shell parsing (use
    `shlex.quote` for each value or a join helper that does it)
- [ ] Import `is_sdk_profile` from
  `squadron.pipeline.summary_oneshot`
- [ ] When `model_raw is None`, behavior is unchanged: no
  `model_switch`, no `command`, the harness uses the session's current
  model
- [ ] Update the returned `ActionInstruction` to include `command` in
  the non-SDK case (the dataclass already has the field; just populate
  it)
  - [ ] SDK profile branch matches the pre-164 behavior exactly
  - [ ] Non-SDK profile branch emits a runnable `sq summary-run …`
    command
  - [ ] No path emits both `model_switch` and `command`

### T12: Test `_render_summary()` profile branching

- [ ] In `tests/pipeline/test_summary_render.py`:
- [ ] **Add** `test_render_summary_sdk_profile_emits_model_switch`:
  - Resolver returns `("haiku-model-id", "sdk")`
  - Assert `model_switch == "/model haiku"` (or whatever the alias was)
  - Assert `command is None`
- [ ] **Add** `test_render_summary_unannotated_alias_emits_model_switch`:
  - Resolver returns `("some-id", None)` — `None` is treated as SDK
  - Same assertions
- [ ] **Add** `test_render_summary_non_sdk_profile_emits_command`:
...
- [ ] **Add** `test_render_summary_non_sdk_profile_quotes_params`:
...
- [ ] **Update** existing tests as needed: any test that asserted
...
- [ ] Run: `uv run pytest tests/pipeline/test_summary_render.py -v`
  - [ ] All new tests pass
  - [ ] Existing tests still pass or are correctly split

### T13: Verify compact-via-summary inheritance

- [ ] Read `src/squadron/pipeline/actions/compact.py` `execute()` SDK
  branch (lines ~203–218) — confirm it still delegates to
  `_execute_summary()` and that no compact-specific changes are needed
- [ ] Spot-check: a compact step with a non-SDK model and stdout/file
...
- [ ] Add a single confirmation test in
...
- [ ] Run: `uv run pytest tests/pipeline/actions/test_compact.py -v`
  - [ ] Compact tests pass
  - [ ] No new compact-specific code was needed

### T14: Full test suite, lint, format, type-check

- [ ] `uv run ruff format .` — confirm no changes (or commit
  formatting in this step)
- [ ] `uv run ruff check` — zero errors
- [ ] `uv run pyright` — zero errors (this slice's predecessor failed
...
- [ ] **Scenario A** — non-SDK summary in SDK execution mode: create
  ...
  minimax model ID
- [ ] **Scenario B** — non-SDK + rotate fails validation: same
  pipeline with `emit: [rotate]`; `sq run --validate` returns the
  expected error
- [ ] **Scenario C** — SDK summary unchanged: pipeline with
  `model: haiku`, `emit: [stdout]`; runs via `sdk_session.capture_summary`
- [ ] **Scenario D** — prompt-only rendering split: SDK pipeline emits
  `model_switch`, non-SDK pipeline emits `command`
- [ ] **Scenario E** — compact-via-summary with non-SDK + stdout: works
- [ ] **Regression** — existing P4 pipeline (haiku stopgap) still
  works identically
  - [ ] All 6 verification scenarios pass
  - [ ] No regression in real-world pipelines

### T16: Commit implementation

- [ ] `uv run ruff format .`
- [ ] `git add` changed files: `src/squadron/pipeline/summary_oneshot.py`,
...
- [ ] `git commit -m "feat: route summary action through provider registry for non-SDK profiles"`
- [ ] If T14 or T15 had to fix anything, fold those into the same
  commit OR a follow-up `fix:` commit — do not leave broken state
  between commits
  - [ ] Commit message follows the project's semantic-prefix
...
- [ ] In `project-documents/user/slices/164-slice.profile-aware-summary-model-routing.md`:
...
- [ ] In `project-documents/user/architecture/140-slices.pipeline-foundation.md`:
  - Mark slice 164's checkbox `[x]` (line 73)
- [ ] Write a DEVLOG entry summarizing:
...
- [ ] `git add -A && git commit -m "docs: mark slice 164 complete"`
  - [ ] Slice frontmatter status flipped to complete
  - [ ] Slice plan checkbox flipped to `[x]`
