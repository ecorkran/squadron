...
- [ ] **T1. Re-verify README's current structure and content**
...
- [ ] **T2. Capture live command output for `sq doctor` and `sq setup`**
...
- [ ] **T3. Verify provider profile table**
...
- [ ] **T4. Create `docs/QUICKSTART.md` — Prerequisites and Install sections**
...
- [ ] **T5. Write "Verify your install" section (`sq doctor` / `sq setup --check-only`)**
...
- [ ] **T6. Write "Configure a provider" section with provider matrix table**
...
- [ ] **T7. Write "Your first review" section**
...
- [ ] **T8. Write "Your first pipeline run" section (net-new content)**
  - Cover `sq run` basics: `sq run --list` to see available pipelines, `sq run <pipeline> <target>` to execute one, `--dry-run`/`--validate` for a safe first try, `--status latest` to check on a run.
  - This content does not exist anywhere in current docs — do not look for an existing section to adapt; write it from T2's live `sq run --help` output and, if needed, `PIPELINES.md` for pipeline-name context.
...
- [ ] **T9. Write "Troubleshooting" and "Windows" sections**
...
- [ ] **T10. Add a single additive pointer in `README.md` to `docs/QUICKSTART.md`**
...
- [ ] **T11. Full top-to-bottom read of `docs/QUICKSTART.md`**
...
- [ ] **T12. Run the verification walkthrough from the slice design**
...
- [ ] **T14. Mark slice complete and write DEVLOG entry**
