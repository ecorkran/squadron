...
file (tasks 1-12) added `list_files`/`grep` tools, the canonical-name translation edge, the
non-SDK unknown-name raise, and the run-scoped injection decision. This file assumes all of
that lands and passes first.

This file: pipeline-action wiring for `review`/`summary` (13-15), tool-use observability
threaded from the agent through `ActionResult.metadata`, `ReviewResult`, and the `-v` log line
(16-20), migrating the seven shipped templates to canonical vocabulary now that the vocabulary
fix actually works (21-22), the live SC6/SC8/SC10 evidence tasks (23-24), and close-out (25).
...
## Task 13: Shared `allowed_tools` resolution helper for review and summary

- [ ] **13.1** Extract a shared helper from `dispatch.py`'s `_resolve_allowed_tools`
  ...
  - [ ] Same contract: read `context.params.get("allowed_tools")`, `None` if absent, raise
    ...
    task 14's schema-side check is the single authority, matching design D3's non-duplication
    rule)
...
## Task 14: Wire `allowed_tools` into `ReviewAction` and validate at load time

- [ ] **14.1** `ReviewAction.validate()` in `review.py` — extend with the same
  ...
  - [ ] If `ReviewAction` does not currently go through a `StepType.validate` path the same way
    ...
    this lands
- [ ] **14.2** `ReviewAction._review` (or equivalent execution method) — read
  `context.params.get("allowed_tools")` via the task 13 helper and pass it to
  `run_review_with_profile` as a new keyword-only parameter, **overriding**
  `template.allowed_tools` when present (a step-level declaration should take precedence over
  the template default — confirm this ordering is what the design intends before implementing;
  if the design is silent, the template stays authoritative when the step declares nothing)
...
## Task 15: Wire `allowed_tools` into `SummaryAction`, test both

- [ ] **15.1** `SummaryAction.validate()` — same `validate_allowed_tools` addition as task 14
- [ ] **15.2** `SummaryAction`'s dispatch-to-model path (`capture_summary_via_profile` call
  around line 249-253) — thread `allowed_tools` from `context.params` through the task 13
...
- [ ] **15.3** Tests: add to `tests/pipeline/actions/test_summary.py`
  - [ ] `test_validate_rejects_unknown_tool`
  - [ ] `test_summary_passes_allowed_tools_to_agent_config`
  - [ ] `test_summary_without_allowed_tools_leaves_field_none`
  - [ ] Add the equivalent two validate/expand-style tests to `tests/pipeline/actions/test_review_action.py`
    for the review-side change from task 14, if not already covered there
  - [ ] Effort: 3/5
...
## Task 16: Tool-use telemetry on the agent's final Message

- [ ] **16.1** In `providers/openai/agent.py`, thread a running tool-call counter through
  ...
  - [ ] Both branches (tool-less fast path and agentic loop) must stamp this identically — a
    test in task 17 asserts they cannot drift
...
## Task 17: Test agent telemetry

- [ ] **17.1** Add to `tests/providers/openai/test_agentic_loop.py`
...
## Task 18: Carry telemetry into `ActionResult.metadata` for dispatch, review, summary

- [ ] **18.1** `one_shot_dispatch` in `dispatch.py` — add a sibling function (e.g.
  ...
  before the join at lines 96-100 discards it, and returns both text and the telemetry dict
  ...
  - [ ] Remove the "slice-265 TODO" comment near this call site if one exists in the current
    ...
    was approximate)
- [ ] **18.2** `ReviewAction` — capture the same telemetry from `run_review_with_profile`'s
  result (task 19 adds the fields to `ReviewResult`) into `ActionResult.metadata`
- [ ] **18.3** `SummaryAction` — capture the same telemetry from whatever
  `capture_summary_via_profile` returns (extend its return shape analogously if needed) into
  `ActionResult.metadata`
  - [ ] Effort: 4/5

- [ ] **Task 18 success criteria**
  - [ ] Dispatch, review, and summary steps that used tools populate
    `ActionResult.metadata["tools_given"]` and `["tool_calls_made"]`
  - [ ] Steps with no tools configured have neither key
  - [ ] `ruff format` run, then committed: `feat: carry tool-use telemetry into ActionResult metadata`

## Task 19: `ReviewResult` telemetry fields and persistence

- [ ] **19.1** Add `tools_given: list[str] | None = None` and
  `tool_calls_made: int | None = None` to the `ReviewResult` dataclass in `review/models.py`
- [ ] **19.2** Add both to `to_dict()` (78-117), in the always-included group (alongside
  `verdict`, `findings`, `model`, etc.), not the verbosity-gated group
- [ ] **19.3** `run_review_with_profile` populates both fields on the returned `ReviewResult`
  from the agent's final-message telemetry (task 16)
...
## Task 20: `-v` log line and end-to-end telemetry tests

- [ ] **20.1** `pipeline/executor.py`, `_log_action_result` (95-108) — add one more `extras`
  ...
  `verdict`/`model`
- [ ] **20.2** Add tests to `tests/pipeline/test_executor.py` (or wherever `_log_action_result`
...
- [ ] **20.3** Integration test proving the full metadata → `-v` line path for at least one
  action type end to end (extend an existing dispatch or review integration test rather than
  building a new harness — check `tests/pipeline/actions/test_dispatch.py` or
  `test_review_action_integration.py` for the closest existing fixture to extend)
  - [ ] Effort: 3/5
...
- [ ] **Task 20 success criteria**
  ...
  - [ ] `ruff format` run, then committed: `feat: render tool-use telemetry in -v log line`

## Task 21: Migrate the seven review templates to canonical vocabulary

- [ ] **21.1** For each of the seven files in `src/squadron/data/templates/`, replace the
  Claude-vocabulary `allowed_tools` list with canonical names per the mapping table (task 7):
  | File | Current | New |
  |---|---|---|
...
## Task 22: SDK-path regression test proving templates are unchanged in effect

- [ ] **22.1** Add a test (in `tests/review/test_templates.py` or `tests/providers/sdk/`,
...
## Task 23: Live non-SDK tool-call integration test (SC6)

- [ ] **23.1** Create/extend a review integration test using a mocked OpenAI-compatible
...
## Task 24: Manual end-to-end verification (SC8, SC10)
...
- [ ] **24.1** Pipeline observability demo
  ...
  - [ ] Check the persisted run JSON at `~/.config/squadron/runs/<run>.json` — confirm
    `tools_given`/`tool_calls_made` appear in `action_results`
- [ ] **24.2** Live non-SDK review (issue #68 closure)
...
## Task 25: Quality gates and close-out

- [ ] **25.1** Run the full gate set
  - [ ] `uv run ruff format .`
  - [ ] `uv run ruff check .`
  - [ ] `uv run pyright` — must be 0 errors. Use `uv run`, not `.venv/bin/pyright`
  - [ ] `uv run pytest -q` — full suite
- [ ] **25.2** Confirm the diff's shape matches the design's Integration Points table: search
...
- [ ] **Task 25 success criteria**
  ...
  - [ ] Diff matches the design's stated scope; no drive-by changes
  - [ ] Every task 1-25 landed its own commit; the history reads as the task sequence
