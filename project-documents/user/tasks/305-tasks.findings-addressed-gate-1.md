...
- [x] **T1. Populate `step_outputs` for loop-body inner steps** (effort 2)
  - [x] In `src/squadron/pipeline/executor.py`, inside `_execute_loop_body`'s
  ...
  - [x] Guard on `step_outputs is not None` (the parameter is optional,
    `executor.py:1326`).
  - [x] Add a comment stating why: a `gate` inside a loop body resolves
    `review_from` through `step_outputs`, and the top-level walk never sees
    inner steps.
  - [x] Success: a gate inside a loop body resolves a named inner review step;
    `most-severe` gates at top level are unaffected.

- [x] **T2. Carry the prior iteration's step outputs into `ActionContext`** (effort 2)
  - [x] In `src/squadron/pipeline/models.py`, add
  ...
  - [x] In `_execute_loop_body`, keep a per-iteration
  ...
  - [x] Thread the new field through the `ActionContext(...)` construction in
...
- [x] **T3. Tests for Part A** (test-with — must pass before Part B starts) (effort 2)
  - [x] In `tests/pipeline/test_executor_loop_body.py`: a loop body whose inner
  ...
  - [x] An inner step that produces no verdict-bearing result does not create a
    `step_outputs` entry.
  - [x] On iteration 1 the action receives an **empty**
  ...
  - [x] A step executed outside any loop receives an empty
    `prior_iteration_step_outputs`.
  - [x] Success: all four pass; full `tests/pipeline/` suite green.
  - [x] **Commit Part A.** `ruff format`, then commit from the project root —
...
- [x] **T4. Gate policy enum and per-policy reference fields** (effort 2)
  - [x] In `src/squadron/pipeline/actions/gate.py`, replace the bare string
  ...
  - [x] Define, once, a mapping from policy → the reference fields that policy
  ...
  - [x] Success: `pyright` strict passes; existing gate tests green with no
    edits.

- [x] **T5. Policy dispatch in `GateAction.execute`** (effort 2)
  - [x] In `actions/gate.py`, dispatch on the resolved policy to a policy
    ...
    no `if policy == "..."` chains.
  - [x] The `most-severe` implementation is the existing body, moved verbatim —
    same outputs, same metadata keys, same log lines.
  - [x] Register **only** `most-severe` here. The `findings-addressed` entry is
  ...
  - [x] Success: `tests/pipeline/test_gate_action.py`,
    `test_gate_reduce.py`, `test_gate_executor.py`, and `test_gate_step.py` pass
    unmodified.

- [x] **T6. Policy-dependent step validation and expansion** (effort 3)
  - [x] In `src/squadron/pipeline/steps/gate.py:23-74`, drive the
  ...
  - [x] Validate the optional `judge:` block: must be a mapping; only `model:`
    is accepted; `model:` must be a string. Reject a `judge:` block on a
    `most-severe` gate — that policy has no model layer.
  - [x] In `expand` (`steps/gate.py:76-93`), build the action dict from the
  ...
  - [x] Pass the `judge:` block through into the gate action's params when
    present.
  - [x] Success: a `findings-addressed` gate step with only `review_from`
    validates and expands to `[gate]` or `[gate, checkpoint]`; a `most-severe`
    gate step expands byte-identically to today.

- [x] **T7. Loader reference resolution per policy** (effort 2)
  - [x] In `src/squadron/pipeline/loader.py:280-307`, resolve which fields to
    check from T4's mapping rather than the hard-coded tuple, so a
    `findings-addressed` gate is not silently unchecked.
  - [x] Leave the top-level-only walk as is. Loop-body gates are validated in
    T10, which is where the body is in scope — do **not** add loop descent to
    the loader.
  - [x] Success: a top-level `findings-addressed` gate whose `review_from` names
    a non-prior step fails at load time with the existing message shape.

- [x] **T8. Tests for the policy config surface** (effort 2)
  - [x] In `tests/pipeline/test_gate_step.py`: `findings-addressed` with only
  ...
  - [x] `judge:` block cases — well-formed passes; non-mapping, unknown key, and
    non-string `model:` each error; a `judge:` block on `most-severe` errors.
  - [x] `expand()` for `findings-addressed` emits no `judge_from` key and does
    not raise; `expand()` for `most-severe` is unchanged.
  - [x] In `tests/pipeline/test_loader.py`: a top-level `findings-addressed`
    gate with a forward/misspelled `review_from` fails validation.
  - [x] Success: all pass; no existing gate/loader test edited.
  - [x] **Commit Part B.** `ruff format`, then commit from the project root —
...
- [x] **T9. Unconsumed-verdict rule in `_validate_verdict_count`** (effort 2)
  - [x] In `src/squadron/pipeline/steps/loop.py:208-242`, change the count from
  ...
  - [x] Collect the consumed names by reading each inner gate step's config
    ...
    names here.
  - [x] Update the docstring to state the rule and why: the gate is the decider,
    its legs are inputs to that decision, and `_last_with_verdict`
    (`executor.py:414-418`) lands on the gate by construction.
  - [x] Leave the error message and its wording unchanged — 910 Part B's
    motivating rejection must fail with the same text.
  - [x] Success: `[dispatch, review, gate]` validates; `[review, review]` with no
    gate still rejects with the existing message.

- [x] **T10. Loop-scoped validation for `findings-addressed`** (effort 3)
  - [x] In `src/squadron/pipeline/steps/loop.py`, add a validation pass over the
    ...
    - [x] rejects the loop when there is **no per-round commit source** — that
      ...
      `UNKNOWN`.
    - [x] rejects the loop when `review_from` does not name an inner step that
      ...
      (`loader.py:285-291`).
  - [x] A `findings-addressed` gate **outside** a loop is not rejected: it is a
    legitimate no-prior-round case that Screen 0 handles observably. Add a
    comment saying so, so a later reader does not "fix" it.
  - [x] Success: the target loop shape validates; the same shape without
    `commit_each_iteration` is rejected at load time with an actionable message.

- [x] **T11. Tests for loop validation** (effort 3)
  - [x] In `tests/pipeline/test_loop_validation.py`: body of
    `dispatch + review(name=fresh-review) + gate(review_from=fresh-review,
    policy=findings-addressed)` validates clean.
  - [x] Body of two `review:` steps with no gate still rejects, asserting on the
    existing message text (910 Part B regression pin).
  - [x] Body of `review + review + gate` naming **one** of them still rejects —
    the unnamed review is unconsumed and the gate's verdict makes two.
  - [x] Body of `review + gate` where the gate names a step that is **not** in
    the body still rejects (the reference is unresolvable, so nothing is
    consumed).
  - [x] The target shape without `commit_each_iteration` is rejected; with it,
    accepted; a phase-bodied loop (which commits on its own) satisfies the
    commit-source requirement.
  - [x] Success: all pass; existing loop-validation tests unchanged.
  - [x] **Commit Part C.** `ruff format`, then commit from the project root —
