...
dependencies: []
projectState: "Implementation complete. Fix 1 (#65) committed bf2dd705. All six issues closed. Slice complete."
status: complete
...
- [x] **Remove** `anthropic` — zero importing files under `src/` (verified
      20260919).
- [x] **Remove** `google-adk` — zero importing files; it pulls a large
      transitive tree into every install.
- [x] **Keep** `mcp` — imported by `tools/mcp_bridge.py` and
      `tools/cf_tools.py`. #65's claim that it is unimported is stale.
- [x] **Add** `rich` with a lower bound at the major version `uv.lock`
      already resolves (currently 14.3.2, arriving transitively via typer).
      21 files under `src/` import it.
- [x] Before editing, re-run the import grep over `src/` to confirm the four
...
- [x] Delete `src/squadron/providers/anthropic/` (`__init__.py`, `agent.py`,
      `provider.py`).
- [x] Delete `src/squadron/adk/`.
- [x] Delete `src/squadron/mcp/` — the real MCP code lives in `tools/`; the
      `mcp` *dependency* stays.
- [x] Each file is three lines: a docstring and
...
- [x] Confirm nothing under `src/`, `tests/`, or `pyproject.toml` references
...
- [x] Confirm `rich` and `mcp` resolve; confirm `anthropic` and `google-adk`
...
- [x] Add the test to `tests/test_smoke.py` — package-level, where
      `test_package_importable` already lives. There is no packaging-specific
      test module and this slice does not create one.
- [x] Assert the declared dependency set: `rich` and `mcp` present,
...
- [x] Confirm the full suite passes with the stub packages deleted — any
...
- [x] Run the design's Verification Walkthrough for Fix 1:
...
- [x] If `pip show anthropic` finds it anyway, another installed package
      requires it transitively — confirm via `uv tree` that `squadron-ai` is
      not the requirer before concluding the removal failed.
- [x] All three `sq` invocations must complete without `ImportError`
      (criterion 3).
- **Effort:** 2
...
- [x] Add a CHANGELOG entry: two dependencies dropped (`anthropic`,
      `google-adk`), `rich` now declared. Keep it a short user-facing bullet;
      technical detail belongs in the DEVLOG.
- [x] This is a **user-visible install change** — the entry is required, not
      optional.
- [x] Commit Fix 1 on its own.
- **Effort:** 1
...
- [x] `ruff format .`
- [x] `ruff check .`
- [x] `pyright` — **zero errors is a merge blocker**.
...
- [x] **1.** `[project.dependencies]` has `rich` and `mcp`, not `anthropic` or
      `google-adk`; `uv.lock` regenerated.
- [x] **2.** `providers/anthropic/`, `adk/`, and `mcp/` no longer exist.
- [x] **3.** A clean-venv base install runs `sq doctor`, `sq run --help`, and
      `sq review --help` without `ImportError`.
- [x] **4.** Gate **passes** when every staged path is outside
      `project-documents/user/` and cf reports `filesChecked: 0`.
- [x] **5.** Gate still **fails closed**, with the D10 message, when ≥1 staged
      path is under `project-documents/user/` and cf reports `filesChecked: 0`.
- [x] **6.** Gate unchanged for `filesChecked > 0`, unreadable count, timeout,
      and missing `cf`.
- [x] **7.** `SubprocessRunner.run` with a nonexistent `cwd` raises
...
- [x] **8.** `GitHubCli` does not report "gh is not on PATH" for a nonexistent
      `cwd`, and logs at WARNING.
- [x] **9.** No `"tool_use"` / `"tool_result"` literal in `translation.py`,
      `sdk_session.py`, `summary_oneshot.py`, `review_client.py`, or
      `metrology/audit.py`.
- [x] **10.** `_ICON[StepKind.INSTALL]` differs from `_ICON[StepKind.CONFIGURE]`
      in both glyph and color, and is not red.
- [x] **11.** A policy-exclusion refusal emits no record at INFO or above; a
      jail-escape refusal still emits exactly one WARNING.
- [x] **Integration.** A release-shaped commit passes the installed pre-commit
...
- [x] Close [#65](https://github.com/ecorkran/squadron/issues/65),
      ...
      fixing commit.
- [x] On #65, note that `mcp` was **kept** and why — the issue text is stale.
- [x] On #100, note that the policy belongs to slice **918 D3**, not 917.
- [x] Leave [#118](https://github.com/ecorkran/squadron/issues/118) open — the
...
- [x] Write a DEVLOG entry per `prompt.ai-project.system.md`, section
...
- [x] Mark the slice complete in
      ...
      entry 20.
- [x] Set `status: complete` in **both** task files —
      `922-tasks.small-fixes-batch-2-1.md` and this one.
- [x] Confirm every checklist item in **both** files is checked — including
