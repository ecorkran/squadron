...
- [ ] **T12. Module skeleton: status enum, evidence types, finding access** (effort 2)
  - [ ] Define `FindingStatus(StrEnum)` with exactly `ADDRESSED`, `UNADDRESSED`,
    `MOVED`, `DISPUTED`. This enum and its parse tokens are defined **once**
    here and referenced everywhere else.
  - [ ] Define `SettlingScreen(StrEnum)` naming which layer settled a finding
    ...
    string.
  - [ ] Define a small frozen dataclass for a per-finding outcome: finding id,
    status, settling screen, optional successor id, optional note.
  - [ ] Add a helper that reads the CONCERN+ subset out of an `ActionResult`'s
  ...
  - [ ] Success: `pyright` strict passes; module imports cleanly with no
    circular-import error from `actions/gate.py`.

- [ ] **T13. Screen 0 — no prior round** (effort 1)
  - [ ] When `context.prior_iteration_step_outputs` has no entry for the
    ...
    iteration.
  - [ ] Never `UNKNOWN` here (design decision 6): a first round is a known state
...
- [ ] **T14. Round diff and Screen 1 — byte-identical round** (effort 3)
  ...
  - [ ] Screen 1: an empty diff **and** empty porcelain output means the round
...
- [ ] **T15. Screen 2 — conservative exact matching** (effort 2)
  ...
  - [ ] Findings whose `location` is `unverified` (904's normalization token for
    ...
    exhaustion.
  - [ ] Matching is exact only. No fuzzy/line-shift tolerance — 911's clean
    regeneration contract moves line numbers wholesale and a false `addressed`
    fails open (design decision 4).
  - [ ] Anything not settled by Screens 0–2 is residue and falls through to the
...
- [ ] **T16. Tests for the deterministic screens** (effort 3)
  ...
  - [ ] Screen 1: fake git reporting an empty diff and empty porcelain → leg
    `FAIL`, every prior CONCERN+ finding `UNADDRESSED` with screen
    `byte_identical`, spy uncalled, WARNING captured.
  - [ ] Git failure: fake git returning non-zero → leg `UNKNOWN`, WARNING names
    the failed command.
  - [ ] Screen 2: prior finding at `src/x.py:12` / `correctness` recurring in the
    fresh set → `UNADDRESSED` via `exact_match`, spy uncalled.
  - [ ] Screen 2 exclusion: two prior/fresh findings both located `unverified`
