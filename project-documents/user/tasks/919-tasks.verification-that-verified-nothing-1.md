...
- [ ] Add a new function in `review/parsers.py` (e.g. `_normalize_line_structure`)
      that inserts a line break before the start of a complete `#{1,6}` hash
      run and after a recognized heading's text, per D1/D2.
- [ ] **Trap 1 (mid-run hash insertion):** anchor insertion on the *start* of
...
- [ ] **Trap 2 (fused heading text):** after inserting the pre-heading break,
      ...
      heading text.
- [ ] Do not yet handle Trap 3 (`location:` anchors) — that is T1.3, and D2
...
- [ ] Extend the normalizer so a `#` immediately preceded by non-whitespace
...
- [ ] Re-measure against the T1.1 fixture: `_locate_section(masked,
      "findings")` must now span all four findings, i.e. `in_section=4`,
      matching `total=4`.
- [ ] Add a dedicated unit test isolating this trap: a synthetic string
...
- [ ] Add three unit tests, each constructed to **fail** if the naive
...
- [ ] Each test's docstring or comment must state which trap it guards and
...
- [ ] Confirm the failure mode by hand first: running `_extract_verdict` on
...
- [ ] Fix `_extract_verdict` ([parsers.py:124](src/squadron/review/parsers.py#L124))
...
- [ ] Re-run the T1.1 fixture end to end (normalize → mask fences → extract
...
- [ ] In `parse_review_output` ([parsers.py:728](src/squadron/review/parsers.py#L728)),
...
- [ ] Confirm the offset contract: `_mask_fences` still receives the
...
- [ ] Confirm the `### Raw Response` writer persists the **original**
...
- [ ] Add a test pinning that a response **containing** newlines is passed
...
- [ ] Run the T1.1 baseline test (still asserting the old broken numbers) —
      confirm it now **fails**, proving the fix changed behavior rather than
      the fixture being unreachable.
- [ ] Update or replace that test to assert the fixed outcome instead
...
- [ ] Run the existing fence-masking test suite unchanged:
      `uv run pytest tests/review/ -k "fence" -v`.
- [ ] Add one new test: a newline-free response whose text *quotes* the
...
- [ ] Add a field to `ReviewResult` (`review/models.py`) carrying whether
