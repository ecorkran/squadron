...
- [x] **T1. Add a scope guard to `review_code`**
  - [x] In `src/squadron/cli/commands/review.py`, in `review_code`, after the
  ...
  - [x] Mirror the existing pattern in `review_slice` (review.py:408-410) /
    `review_tasks` (review.py:551-553); do not invent a new validation style.
  - [x] Distinguish the malformed case: when `slice_number is not None` but
  ...
  - [x] **Success:** with no scope, the guard exits before any model call; a
    valid slice number, `--diff`, or `--files` still proceeds normally.
  - Effort: 2/5

- [x] **T2. Test the scope guard** *(test-with T1)*
  - [x] Add CLI tests (invoke `review_code` via the typer test runner / direct
  ...
  - [x] **Success:** all four cases pass; the not-called assertion proves no
    fabricated-review path remains for missing/malformed scope.
  - Effort: 2/5

- [x] **T3. Commit Part C**
  - [x] `ruff format`, run the Part C tests, then commit
    (`fix: guard sq review code against missing/malformed slice scope`).
  - [x] Reference issue #17 in the commit body.
  - Effort: 1/5
...
- [x] **T4. Add `name` to `ProjectInfo` and populate it**
  - [x] In `src/squadron/integrations/context_forge.py`, add `name: str` to the
    `ProjectInfo` dataclass (~line 52).
  - [x] In `get_project` (~line 165), populate it from `data.get("name")`
    (verified: `cf get --json` returns a `name` field). Use a non-silent
    fallback of `"unknown"` if the key is absent — never `"squadron"`.
  - [x] **Success:** `get_project().name` returns the real project name;
    `ProjectInfo` construction still type-checks (pyright strict).
  - Effort: 1/5

- [x] **T5. Test `ProjectInfo.name` population** *(test-with T4)*
  - [x] Add/extend a test for `get_project` using a **real-shaped** `cf get
  ...
  - [x] **Success:** both cases pass; fixture matches the actual CLI shape.
  - Effort: 1/5

- [x] **T6. Thread `project` through `SliceInfo` / `resolve_slice_info`**
  - [x] In `src/squadron/review/persistence.py`, add `project: str` to the
    `SliceInfo` TypedDict (~line 20).
  - [x] In `resolve_slice_info` (~line 66, which already calls
    `cf_client.get_project()`), set `project=project.name` in the returned
    `SliceInfo`.
  - [x] **Success:** `resolve_slice_info(...)["project"]` returns the real name;
    existing callers still type-check.
  - Effort: 1/5

- [x] **T7. Replace the hardcoded `project: squadron` literal**
  - [x] In `format_review_markdown` (persistence.py ~line 119), replace the
  ...
  - [x] **Success:** the review file's `project:` field reflects the actual
    project; a `None` `slice_info` yields `project: unknown`.
  - Effort: 1/5

- [x] **T8. Test the frontmatter project field (both write paths)** *(test-with T6/T7)*
  - [x] Test `format_review_markdown` directly: with a `SliceInfo` carrying
  ...
  - [x] Interface-parity assertion: since both the pipeline path
    ...
    `project:` value.
  - [x] **Success:** all cases pass; no path emits a hardcoded `squadron`.
  - Effort: 2/5

- [x] **T9. Commit Part B**
  - [x] `ruff format`, run Part B tests, commit
    (`fix: derive review frontmatter project from cf, not hardcoded squadron`).
  - [x] Reference issue #16 in the commit body.
  - Effort: 1/5
...
- [x] **T10. Add `expected_artifact_kind` to `PhaseStepType`**
  - [x] In `src/squadron/pipeline/steps/phase.py`, add an `expected_artifact_kind`
  ...
  - [x] The three registrations (design/tasks/implement, phase.py:133-135) must
    each resolve to the correct kind; `implement` is `None` (no single
    deterministic artifact → post-condition does not apply).
  - [x] **Success:** `PhaseStepType("tasks").expected_artifact_kind` is the
    task-file kind; `PhaseStepType("implement").expected_artifact_kind` is
    `None`.
  - Effort: 2/5

- [x] **T11. Test `expected_artifact_kind` mapping** *(test-with T10)*
  - [x] Parametrized test over the three phases asserting the exact kind for
    each, including `implement` → `None`.
  - [x] **Success:** mapping test passes; adding a hypothetical new phase with
    no kind would default to `None` (assert the property does not raise for an
    unmapped phase name).
  - Effort: 1/5

- [x] **T12. Add a run-start timestamp accessor for the executor**
  - [x] Confirm `RunState.started_at` (`state.py:126`) and that the executor
  ...
  - [x] Decide the anchor: prefer loading `started_at` once per step (or reuse
  ...
  - [x] **Success:** the executor has access to a UTC run-start timestamp at the
    point the post-condition runs, without adding a field to `ActionContext`.
  - Effort: 2/5

- [x] **T13. Implement the artifact post-condition in the executor**
  - [x] In `_execute_step_once` (`executor.py`), in the per-action tail after a
  ...
  - [x] Enumerate every failure mode from the design's table with an observable
    ...
    calls only.
  - [x] Do **not** modify generic `DispatchAction` — the check is executor-owned
    and scoped to phase steps with a non-`None` kind; bare `dispatch` steps and
    `implement` phases are untouched.
  - [x] **Success:** a phase-step dispatch that writes no artifact yields a
...
- [x] **T14. Test the artifact post-condition** *(test-with T13)*
  - [x] Unit/integration tests over the executor seam: (a) artifact present with
  ...
  - [x] Assert generic `dispatch` (non-phase) is unaffected — a bare dispatch
    step that writes nothing still succeeds.
  - [x] **Success:** all six cases pass; each failure mode produces its expected
    observable signal (failed step + log).
  - Effort: 3/5

- [x] **T15. Route the no-artifact outcome to checkpoint/escalation**
  - [x] Ensure the failed-step outcome from T13 flows into the existing
  ...
  - [x] **Success:** in a pipeline with `on_fail`/checkpoint configured, a
    no-artifact phase step triggers the configured failure behavior instead of
    silently advancing.
  - Effort: 2/5

- [x] **T16. Test no-artifact → checkpoint/on-fail routing** *(test-with T15)*
  - [x] Integration test: run a pipeline containing a phase step whose dispatch
  ...
  - [x] Assert the observable signal: the configured on-fail path fires (e.g.
    checkpoint pause recorded / pipeline stops), not silent advancement.
  - [x] **Success:** the test proves a no-artifact phase step stops the run via
    the existing failure routing rather than continuing to a downstream step.
  - Effort: 2/5

- [x] **T17. Commit Part A**
  - [x] `ruff format`, run Part A tests, commit
    (`fix: fail phase step when dispatch writes no expected artifact`).
  - [x] Reference issue #15 in the commit body.
  - Effort: 1/5
...
- [x] **T18. Full validation gate + verification walkthrough**
  - [x] Run the full test suite + `ruff format` + pyright strict; zero errors.
  - [x] Execute the design's Verification Walkthrough for all three parts:
    ...
    later).
  - [x] Close issues #15, #16, #17 on merge; mark slice 909 complete in the
