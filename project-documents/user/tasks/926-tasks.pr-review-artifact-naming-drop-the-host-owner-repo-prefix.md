...
dependencies: []
projectState: Slice complete. All parts implemented and committed on branch 926-slice.pr-review-artifact-naming-drop-the-host-owner-repo-prefix; full suite green (4461 passed), pyright clean; walkthrough verified with PR 111 (PR 116's base branch was deleted).
status: complete
...
- [x] Effort: 1/5
- [x] In [src/squadron/review/reviews_dir.py](src/squadron/review/reviews_dir.py),
      ...
      - `CONFIG` → `False`
      - `FLAG` → `False`
- [x] Docstring states the property is the single place this mapping lives
...
- [x] Effort: 1/5
- [x] In `tests/review/test_reviews_dir.py` (or create it if no such file
...
- [x] The point: adding a new rule without deciding its `repository_scoped`
      value must fail this test, not silently default.
- [x] Add a second, simple test asserting the exact True/False mapping from
      D1's table for all four current members.
- [x] Run: `pytest tests/review/test_reviews_dir.py -x`.

### Task A.3 — Commit Part A

- [x] Effort: 1/5
- [x] Confirm current working directory is the squadron project root.
- [x] `git add` and commit Part A's changes. Per CLAUDE.md, commit at least
...
- [x] Effort: 3/5
- [x] In [src/squadron/cli/commands/review_pr.py](src/squadron/cli/commands/review_pr.py),
      ...
      `self._qualify`.
- [x] Replace `filename_stem` (currently line 144:
...
- [x] Do not touch `PullRequestRecord.path_key` or `.key` in
      [src/squadron/codehost/models.py](src/squadron/codehost/models.py) in
      this task — that's Task D.1.
- [x] Move the `resolve_reviews_dir(...)` call (currently inside `_save_pr`,
...
- [x] Pass `qualify=not rule.repository_scoped` into the `PrTarget(...)`
      construction at (what is currently) line 471 — the only call site.
- [x] Update `_save_pr` (lines 432–464) to take the pre-resolved
...
- [x] Verify no behavior change to the printed "Saved review to … (<rule>)"
      line (line 463) or the `OSError` failure path (lines 457–462).

### Task B.2 — Update `PrTarget` stem tests

- [x] Effort: 2/5
- [x] In `tests/cli/test_review_pr_persistence.py`, update every test that
...
- [x] Add new tests (do not replace, add) covering:
...
- [x] Run: `pytest tests/cli/test_review_pr_persistence.py -x`.
...
- [x] Effort: 3/5
- [x] In `tests/cli/test_review_pr_persistence.py`, add or extend
      ...
      - `FLAG` (`--reviews-dir <tmp>`) →
        `pr-{number}-review.code.{owner}-{repository}.md`, qualified.
- [x] Each case must go through the real `review_pr` CLI path (not call
      `PrTarget` or `resolve_reviews_dir` directly) — the point is proving
      the two are wired together correctly end to end.
      - Implemented in tests/cli/test_review_pr.py (test_each_reviews_dir_rule_decides_the_artifact_qualifier), where the CliRunner + faked code host fixtures live; test_review_pr_persistence.py has no CLI host fake. Mutation-checked: wiring qualify to FLAG-only fails the CONFIG case.
- [x] Run: `pytest tests/cli/test_review_pr_persistence.py -x`.
...
- [x] Effort: 1/5
- [x] Confirm current working directory is the squadron project root.
- [x] `git add` and commit Part B's changes.
...
- [x] Make no changes to either file. If either test fails after Parts A/B's
      ...
      - tests/review/test_pr_artifact_is_target_agnostic.py: _pr_target() helper gained qualify=False (required by the new keyword-only parameter). Its github.com- fixtures are untouched.
- [x] Run: `pytest tests/documents/test_pr_review_frontmatter.py tests/review/test_pr_artifact_is_target_agnostic.py -x`.

### Task C.3 — Commit Part C

- [x] Effort: 1/5
- [x] Confirm current working directory is the squadron project root.
- [x] `git add` and commit Part C's changes (the widened test only —
...
- [x] Effort: 1/5
- [x] In [src/squadron/codehost/models.py](src/squadron/codehost/models.py),
...
- [x] Do not change `path_key`'s implementation (line 65,
      `self.key.replace("/", "-").replace("#", "-")`) — only the docstring.
- [x] Confirm `ScratchWorktree.__enter__` is still the only remaining
...
- [x] Effort: 1/5
- [x] In [src/squadron/metrology/capture.py](src/squadron/metrology/capture.py),
...
- [x] This is a message-only change — `resolve_target`'s behavior is
...
- [x] Effort: 1/5
- [x] Find or create the test covering `resolve_target`'s non-digit,
      non-path refusal (check `tests/metrology/` for an existing test
      exercising this branch before adding a new one).
- [x] Assert the error message contains "PR reviews are always addressed by
      path".
- [x] Run: `pytest tests/metrology/ -k resolve_target -x`.

### Task D.4 — Commit Part D

- [x] Effort: 1/5
- [x] Confirm current working directory is the squadron project root.
- [x] `git add` and commit Part D's changes.
...
- [x] Effort: 1/5
- [x] In `tests/pr/` (check for an existing test file covering
...
- [x] This is a regression pin per D5: the slice plan wrongly assumed this
      ...
      discovery.
      - Pinned through the real find_latest_in_range_review (tests/pr/test_inputs.py::test_discovery_glob_finds_every_pr_review_name_form) rather than a copied glob pattern, so tightening the glob in inputs.py fails the test.
- [x] Run: `pytest tests/pr/ -k glob -x`.

### Task E.2 — Commit Part E

- [x] Effort: 1/5
- [x] Confirm current working directory is the squadron project root.
- [x] `git add` and commit Part E's changes.
...
- [x] Effort: 1/5
- [x] In `docs/COMMANDS.md`, find the `sq review pr` section and add or
...
- [x] Effort: 1/5
- [x] This edit lands in a **different repository**:
...
- [x] In that repo's `file-naming-conventions.md`, find the "Pull-Request
      ...
      prefix.
- [x] Commit this change in the `ai-project-guide` repo, separately from
...
- [x] Confirm current working directory is the squadron project root
      ...
      separately).
- [x] `git add` and commit `docs/COMMANDS.md` from F.1.
...
- [x] Effort: 1/5
- [x] From the squadron repo root: `ruff format && ruff check && pyright`.
      Zero pyright errors — merge blocker per project rules.
- [x] Run the full test suite: `pytest`. All green.

### Task G.2 — Manual verification walkthrough

- [x] Effort: 2/5
- [x] Follow the design's Verification Walkthrough section exactly (design
      ...
      6. Clean up the step-1 artifact (`git checkout` or `rm`) unless
         intentionally kept.
      - PR 116 unfetchable (base branch squadron-pr deleted); used PR 111. All steps matched. Step 1 verdict was UNKNOWN (review parse, unrelated to naming). Step 4 path form resolves, then requires --verdict when stdin is non-interactive.
- [x] Note any deviation from the design's expected output as a finding,
...
- [x] Effort: 1/5
- [x] Confirm current working directory is the squadron project root.
- [x] `git status` — working tree clean (Parts A–F already committed
...
- [x] `git log --oneline` over this slice's commits reads as one coherent
...
- [x] Mark this task file's `status:` as `complete` in frontmatter once all
      parts are done (or delegate to `task-checker`).
- [x] Mark the slice design's `status:` as `complete` in
      [926-slice.pr-review-artifact-naming-drop-the-host-owner-repo-prefix.md](project-documents/user/slices/926-slice.pr-review-artifact-naming-drop-the-host-owner-repo-prefix.md)
      frontmatter.
- [x] Update the slice plan entry (900-slices.maintenance-and-refactoring.md,
      entry 24 / index 926) checkbox to `[x]`.
- [x] Write a DEVLOG entry per `prompt.ai-project.system.md`, Session State
